// Fetches a real Bluesky account's own posts via the public AppView. Copy of
// the read pattern in sites/megahal/public/lib/you-brain.js, itself trimmed
// from sites/spellclash/public/lib/atproto.js — same shape, reused here so
// two independent handles can each be pulled and trained separately.
...
export async function fetchProfile(actor) {
  return jget(`${PUB}/app.bsky.actor.getProfile?actor=${encodeURIComponent(normalizeActor(actor))}`);
}

// Pages through app.bsky.feed.getAuthorFeed collecting original-post text
// (no reposts, no replies) until it has `maxPosts` lines or runs out of
// feed. Page budget scales with maxPosts (100 raw items/page, some of which
// are replies/reposts we discard) plus slack, capped so a huge maxPosts
// can't turn into an unbounded sequential-fetch loop against someone's
// account.
export async function fetchAuthorLines(actor, maxPosts = 2000, onPage) {

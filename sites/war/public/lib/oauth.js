// oauth.js — browser-side atproto OAuth (public client), for war.bisks.net.
//
// Copied from keytags/public/lib/oauth.js (same repo), itself trimmed from
...
export async function login(handleOrDid) {
  ...
  // the auth server's callback can't carry along whatever path/query the user
  // was on when they clicked "sign in". We stash that here as `returnTo` and

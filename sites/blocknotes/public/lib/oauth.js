// oauth.js — browser-side atproto OAuth (public client), for blocknotes.bisks.net.
//
// Forked from sites/mutualsky/public/lib/oauth.js (same repo), itself
// descended from sites/skyclone / drivethru / mino.mobi's airchat oauth/flow.js.
// Public client:
...
//
// Dedicated subdomain, served at the root, so MOUNT is always "".
//
// Scope is narrowed to exactly what blocknotes touches: create+delete on
// app.bsky.graph.block (real blocks) and on this site's own
// net.bisks.blocknotes.entry collection (the date+reason note kept alongside
// each block/mute), plus three rpc grants proxied through the user's own PDS
// to the AppView for the mute side — muting/unmuting isn't a repo record,
// it's account-side AppView state, and listing current mutes needs an
// authenticated read there too.
//
// If narrow rpc:/repo: scopes ever get rejected by a user's PDS as
// invalid_scope, the safe fallback is reverting this SCOPE and
// client-metadata.json's scope back to `atproto transition:generic` for this
// one site — see notes/50-oauth-scopes.md.
...
const ORIGIN = location.origin; // https://blocknotes.bisks.net (or localhost in dev)
const MOUNT = ""; // canonical host is blocknotes.bisks.net, served at the root
...
const SCOPE =
  "atproto repo:app.bsky.graph.block?action=create&action=delete repo:net.bisks.blocknotes.entry?action=create&action=delete rpc:app.bsky.graph.muteActor?aud=did:web:api.bsky.app%23bsky_appview rpc:app.bsky.graph.unmuteActor?aud=did:web:api.bsky.app%23bsky_appview rpc:app.bsky.graph.getMutes?aud=did:web:api.bsky.app%23bsky_appview";
...
const DB_NAME = "blocknotes-oauth";
const STORE = "session";
...
export async function login(handleOrDid) {
  ...
  // same-origin; the redirect returns to this same tab.
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page the user started the login from.
...
//
// The DPoP proof's `htu` claim must be the target URI *without* the query
// string (RFC 9449 §4.2) — stripped here so callers can pass query params
// straight through without hand-rolling two URLs.
export async function dpopFetch(session, url, options = {}) {
  ...
  const doFetch = async (nonce) => {
    const dpop = await deserializeDPoPKeyPair(session.dpop);
    const proof = await createDPoPProof(dpop, method, htu, nonce, session.accessJwt);
    return fetch(url, {
      ...options,
      headers: {
        ...(options.headers || {}),

// oauth.js — browser-side atproto OAuth (public client), for tallybot.bisks.net.
//
...
// narrowed to our own net.bisks.tallybot.point collection, create-only — no
// account-wide access, no update/delete (a vote is permanent once cast).
...
const ORIGIN = location.origin; // https://tallybot.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is tallybot.bisks.net — brand new site, no legacy path route
...
export const SCOPE = "atproto repo:net.bisks.tallybot.point?action=create";
...
export async function login(handleOrDid) {
  ...
  // established, so signing in from a tally page takes you back to that
  // same tally instead of dumping you on the home page.
...
export async function completeLoginIfCallback() {
  ...
  // three real documents (/, /leaderboard/, /t/<key>), not one SPA shell,

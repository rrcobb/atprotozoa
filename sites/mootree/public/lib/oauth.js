// oauth.js — browser-side atproto OAuth (public client), for mootree.bisks.net.
//
// Copied and trimmed from padmoot/public/lib/oauth.js (same repo), itself
// adapted from paintmoot/trigrams' oauth.js / mino.mobi's airchat/oauth/flow.js.
// Public client:
...
//
// The client_id is the URL of our static client-metadata.json. This site only
// *reads* (the user's own follow records + public profiles), so the scope is
// just `atproto` — no transition:generic, no writes to anyone's repo.
...
const ORIGIN = location.origin; // https://mootree.bisks.net (or localhost in dev)
// This site owns its whole host — no mount prefix to thread through.
export const MOUNT = "";

...
const CACHE_KEY = "steamtags:global-index:v1";
...
function normaliseRecord(did, rkey, record) {
  ...
  const tags = (Array.isArray(record.tags) ? record.tags : [])
    .filter((tag) => tag && typeof tag.tag === "string" && tag.tag.trim() && Number.isFinite(tag.score))
    .slice(0, MAX_TAGS_PER_ENTRY)
    .map((tag) => ({
      tag: tag.tag.trim().slice(0, MAX_TAG_NAME),
      score: Math.max(1, Math.min(10, Math.round(tag.score))),
      votes: Number.isFinite(tag.votes) ? Math.max(0, Math.round(tag.votes)) : 0,
    }));
  ...
  return {
    ...
    headerImage: typeof record.headerImage === "string" ? record.headerImage.slice(0, 500) : "",
    tags,
    updatedAt: Date.now(),
  };
}
...
export class GlobalIndex {
  ...
  emit() {
    try {
      ...
      console.error("steamtags global render failed", err);
    }
  }
  ...
  async runBackfill() {
    ...
    try {
      ...
      retryMs = 5000;
      console.warn("steamtags global backfill failed", err);
    } finally {
      this.backfillRunning = false;
      this.scheduleEmit();
  ...
  buildTagBoards() {
    ...
    for (const entry of this.entries.values()) {
      for (const tag of entry.tags) {
        let bucket = tagMap.get(tag.tag);
        if (!bucket) {
          bucket = { scores: [], users: new Set(), games: new Map() };
          tagMap.set(tag.tag, bucket);
        }
        bucket.scores.push(tag.score);
        bucket.users.add(entry.did);
        let game = bucket.games.get(entry.appid);
        if (!game) {
          game = { name: entry.name, headerImage: entry.headerImage, scores: [], users: new Set() };
          bucket.games.set(entry.appid, game);
        }
        game.scores.push(tag.score);
        game.users.add(entry.did);
        if (entry.name) game.name = entry.name;
        if (entry.headerImage) game.headerImage = entry.headerImage;
      }
    }

    const average = (values) => values.reduce((sum, value) => sum + value, 0) / values.length;
    const boards = Array.from(tagMap.entries()).map(([tag, bucket]) => {
      const games = Array.from(bucket.games.entries())
        .map(([appid, game]) => ({
          appid,
          name: game.name,
          headerImage: game.headerImage,
          avgScore: average(game.scores),
          raterCount: game.users.size,
        }))
    ...
    return boards
      .sort((a, b) => b.avgScore - a.avgScore || b.ratingCount - a.ratingCount)

// whodatninja Worker — served at the root of whodatninja.bisks.net. The game
// itself (loading a handle's real moots, running the lineup quiz) is
// entirely client-side in public/ — see public/lib/spy-data.js. Copied from
// sites/mootspy/src/index.ts (copy, don't abstract); this Worker has the
// same two jobs:
//
...
//    notes/45-sharing-and-virality.md, tier 4): resolves the handle
//    server-side and stamps a personalized title/description onto the same
//    static shell before serving it.
...
async function renderShare(env: Env, request: Request, rawHandle: string): Promise<Response> {
  ...
  try {
    ...
    const title = `who dat ninja? guess ${who}'s moots by face`;
    const desc =
      mootCount > 0
        ? `a mystery pfp from ${who}'s ${mootCount} real moots shows up with no name on it. pick the right @handle from a lineup — no display names to give it away.`
        : `mystery pfps from ${who}'s network show up with no name on them. pick the right @handle from a lineup — no display names to give it away.`;
    const url = `https://whodatninja.bisks.net/s/${encodeURIComponent(handle)}`;

    html = html
      .replace(/<title>.*?<\/title>/, `<title>${esc(title)} — whodatninja</title>`)

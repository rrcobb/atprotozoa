// metamuseum Worker — served at the root of metamuseum.bisks.net.
//
...
async function stampedShell(env: Env, request: Request, title: string, desc: string, path: string): Promise<Response> {
  ...
  const stamped = stampMeta(html, title, desc, `https://metamuseum.bisks.net${path}`);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (pieceMatch) {
      ...
      if (piece) {
        const desc = `${piece.blurb} Commissioned by ${piece.by || "person unknown"}. Full wall text, provenance, and critical reception at the Metamuseum of the Internet.`;
        return stampedShell(env, request, `metamuseum: ${piece.title}`, desc, `/piece/${encodeURIComponent(piece.name)}`);
      }
    }
    ...
    if (wingMatch) {
      ...
      if (meta) {
        ...
        return stampedShell(env, request, `metamuseum: the ${meta.label} wing`, desc, `/wing/${encodeURIComponent(key)}`);
      }
    }

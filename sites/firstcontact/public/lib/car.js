...
// cursor walk one page at a time. One request gets a moot's *entire* post
// history in one shot (no per-moot page cap) — this is the trick that
// replaced backscroll's old paginated app.bsky.feed.getAuthorFeed walk (see
...
// data"). See sites/activitygrid, where this first shipped, for the full
// writeup of why it beats listRecords/getAuthorFeed for "give me this
// person's whole history."
//
// Copy, don't abstract: this is the keyed variant of the CAR/DAG-CBOR parser
// from sites/beefcheck's lib/car.js (copied verbatim via sites/ngmi, not
// imported across sites), trimmed to just fetchRepoRecordsWithKeys —
// backscroll needs every post's rkey to build a permalink back to the exact
// post, which a flat block scan can't recover: a record's key
// ("collection/rkey") only exists as an edge label in the repo's MST (Merkle
// Search Tree), not in the record block itself. This walks the MST from the
// commit's root to reconstruct every key.
...
//
// `types` may be omitted/null/undefined to mean "every collection" — needed
// here (unlike backscroll, which only ever wants app.bsky.feed.post) because
// firstcontact has to see whatever's actually in the repo, including
// third-party lexicons (a streamplace chat message, a semble card link, …)
// it has no fixed list of $type strings for.
  ...
  function walk(nodeKey) {
    ...
    for (const entry of node.e || []) {
      ...
      if (recBytes) {
        ...
        if (rec && (wanted === null || wanted.has(rec.$type))) {

...
// rateyourbuild.bisks.net. Copied and trimmed from
// sites/steamtags/public/lib/oauth.js (same repo).
//
// Public client:
...
// narrowed to create+update+delete on our own net.bisks.rateyourbuild.rating
// collection only — no account-wide access. Must stay in lockstep with
// client-metadata.json (see notes/50-oauth-scopes.md).
...
const ORIGIN = location.origin; // https://rateyourbuild.bisks.net (or localhost in dev)
// Root-mounted on its own subdomain — no mount prefix needed.
export const MOUNT = "";
...
const SCOPE = "atproto repo:net.bisks.rateyourbuild.rating";
...
const DB_NAME = "rateyourbuild-oauth";
const STORE = "session";
...
export async function completeLoginIfCallback() {
  ...
  let dpopProof = await createDPoPProof(dpop, "POST", s.tokenEndpoint, s.dpopNonce || undefined);
...
export async function refreshSession(session) {
  ...
  session.accessExpiresAt = jwtExp(tokens.access_token) || Math.floor(Date.now() / 1000) + (tokens.expires_in || 3600);

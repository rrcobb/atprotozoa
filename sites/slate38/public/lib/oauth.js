// oauth.js — browser-side atproto OAuth (public client), for slate38.bisks.net.
//
...
const ORIGIN = location.origin; // https://slate38.bisks.net (or localhost in dev)
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page the user started the login from — this site
  // is one document (/), so a plain history.replaceState is enough (no
  // cross-document navigation needed the way influential25's three-document
  // site requires).

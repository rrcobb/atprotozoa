// oauth.js — browser-side atproto OAuth (public client), for kinesin.bisks.net.
//
...
//
// Scope: narrowed per notes/50-oauth-scopes.md — kinesin only ever creates
// its own net.bisks.kinesin.walk run records, nothing else. Must match
...
const ORIGIN = location.origin; // https://kinesin.bisks.net (or localhost in dev)
const MOUNT = "";
...
const SCOPE = "atproto repo:net.bisks.kinesin.walk?action=create";
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page the user started the login from — kinesin's
  // OAuth redirect always comes back to REDIRECT_URI (the root), so if the
  // user started the flow from /leaderboard a plain history.replaceState
  // would leave the already-loaded root document sitting under the wrong
  // URL; a real navigation is needed so the browser actually fetches that
  // page's HTML.

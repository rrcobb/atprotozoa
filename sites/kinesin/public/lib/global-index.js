// global-index.js — network-wide view over a kinesin collection:
...
// which already takes the collection name and a per-record normalizer.
//
...
export class GlobalIndex {
  // collection: full NSID string, e.g. "net.bisks.kinesin.walk"
  ...
  constructor(collection, { normalize, onUpdate } = {}) {
    ...
    this.cacheKey = `kinesin:global-index:${collection}:v1`;
    ...
    // doesn't get silently double-applied across a page reload.
  ...
  emit() {
    try {
      ...
      console.error("kinesin global-index render failed", err);
    }
  }
  ...
  async runBackfill() {
    ...
    try {
      ...
      retryMs = 5000;
      console.warn("kinesin global-index backfill failed", err);
    } finally {
      this.backfillRunning = false;
      this.scheduleEmit();

// oauth.js — browser-side atproto OAuth (public client), for revolver.bisks.net.
//
...
//
// revolver is a dedicated subdomain (no bisks.net/ path mount), so
// client_id/redirect_uri are just the origin — no mount-prefix detection
// needed here, unlike the path-mounted sites this was copied from.
//
...
// account-wide access. Must stay in lockstep with client-metadata.json. Every
// player has to grant this scope before they can load a dare, because either
// side of the round might turn out to be the one whose account posts.
...
const ORIGIN = location.origin; // https://revolver.bisks.net (or localhost in dev)

// oauth.js — browser-side atproto OAuth (public client), for bskyxp.bisks.net.
//
// Copied and adapted from sites/skyclone/public/lib/oauth.js (same repo),
// itself descended from sites/drivethru's / mino.mobi's airchat/oauth/flow.js.
// Public client:
...
//
// A dedicated subdomain, so location.origin alone is the client_id/redirect_uri
// — no mount prefix to bake in.
//
// Scope is narrowed to exactly what bskyxp touches: create-only on
...
// timeline, notifications list/seen/unread-count). Everything else this
// client reads (profiles, threads, search, other people's feeds) goes
...
const ORIGIN = location.origin; // https://bskyxp.bisks.net (or localhost in dev)
const MOUNT = "";

// oauth.js — browser-side atproto OAuth (public client), for tacocounter.bisks.net.
//
...
//
// Scope: narrowed per notes/50-oauth-scopes.md — tacocounter only ever
// creates its own net.bisks.tacocounter.{log,board,membership} records,
// nothing else. Must match public/client-metadata.json's `scope` field
// exactly.
...
const ORIGIN = location.origin; // https://tacocounter.bisks.net (or localhost in dev)
const MOUNT = "";
...
const SCOPE =
  "atproto repo:net.bisks.tacocounter.log?action=create repo:net.bisks.tacocounter.board?action=create repo:net.bisks.tacocounter.membership?action=create";
...
const DB_NAME = "tacocounter-oauth";
const STORE = "session";
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page the user started the login from (e.g. a
  // specific /b/<did>/<rkey> board page) — a plain history.replaceState
  // would leave the already-loaded root document sitting under the wrong
  // URL, so a real navigation is needed when returnTo differs.

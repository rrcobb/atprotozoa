// oauth.js — browser-side atproto OAuth (public client), for subatomic.bisks.net.
//
// Copied and trimmed from sites/thewell/public/lib/oauth.js (same repo),
// itself from sites/hyperobject/velvetrope/clusterpedia/padmoot, adapted from
...
// three narrow `repo:` grants: create-only on net.bisks.subatomic.post and
// net.bisks.subatomic.comment, and full create/update/delete on
// net.bisks.subatomic.vote (a vote can be cast, changed, or withdrawn) — not
// the old `transition:generic` app-password-equivalent scope, which would
// hand this site read/write over the signer's *entire* repo. Every write
// writes a record to the *acting user's own* PDS; the Worker never trusts a
...
// feed (see src/index.ts's verifyOwnRecord / verifyRecordAbsent).
...
const ORIGIN = location.origin; // https://subatomic.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is subatomic.bisks.net — no path mount
...
const SCOPE =
  "atproto repo:net.bisks.subatomic.post?action=create repo:net.bisks.subatomic.comment?action=create repo:net.bisks.subatomic.vote";

const BSKY_PUBLIC_API = "https://api.bsky.app";
const PLC_DIR = "https://plc.directory";

// A voter's deterministic per-subject rkey — sha256(subject at-uri) hex,
// first 32 chars. Casting a new vote on the same post reuses this rkey via
// putRecord (overwrite, not a second record); src/index.ts's expectedRkey()
// recomputes the same value server-side to enforce it.
export async function voteRkey(subjectUri) {
...
export async function login(handleOrDid) {
  ...
  // established, so signing in from a post takes you back to that post.
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page (post, thread, ...) the user started the

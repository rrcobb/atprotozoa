// oauth.js — browser-side atproto OAuth (public client), for bisks.net/postwith.
//
// Copied and trimmed from sites/padmoot/public/lib/oauth.js (same repo, house
// style: copy don't abstract). Public client:
...
// narrowed to our own net.bisks.postwith.* collections only (full CRUD on
// .profile, create-only on .meeting/.response/.feedback) — no account-wide
...
// Canonical host is postwith.bisks.net, so there is no mount prefix. The site is
// still reachable at bisks.net/postwith, but OAuth only works on the canonical
// host: client_id must equal the URL the metadata is actually served from.
export const MOUNT = ""; // canonical host is postwith.bisks.net — see audit/migrate-oauth-hosts.mjs
...
const SCOPE =
  "atproto repo:net.bisks.postwith.profile repo:net.bisks.postwith.meeting?action=create repo:net.bisks.postwith.response?action=create repo:net.bisks.postwith.feedback?action=create";
...
export async function login(handleOrDid) {
  ...
  // the auth server's callback can't carry along whatever page/query the user
  // was looking at when they clicked "sign in". We stash that here as
  // `returnTo` and completeLoginIfCallback() navigates back to it once the
  // session is established, instead of dumping the user on /.

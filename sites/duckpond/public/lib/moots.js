// moots.js — turn a Bluesky handle into its "moots" (mutuals) so we can fill
// a pond with ducklings it actually knows.
//
// moots = MUTUALS: the accounts a handle follows that also follow back
// (follows ∩ followers). A roster of mutuals makes the pond recognizable —
// you know the faces waddling behind you. If there aren't enough mutuals to
// make a decent-sized flock, we widen to plain follows so a game can still run.
//
...
// Copied and trimmed from simcluster/moots.js (copy, don't abstract).
...
const MIN_POOL = 24; // below this, widen mutuals → follows so a decent-sized flock still forms
...
// `pool` is the moots WITHOUT self (self is handed back separately — it's
// mama duck). `kind` is "moots" or "moots + follows" (the widen fallback).
export async function moots(actor, { onStep } = {}) {
  ...
  // self: display name / avatar for mama duck ("you").
  let self = {
    did,
  ...
  if (pool.length < MIN_POOL) {
    // widen: add plain follows (recognizable enough) to reach a decent-sized flock
    for (const f of follows) {

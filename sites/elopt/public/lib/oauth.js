// oauth.js — browser-side atproto OAuth (public client), for elopt.bisks.net.
//
...
//
// Scope is two narrow `repo:` grants (notes/50-oauth-scopes.md), not the old
// `transition:generic` app-password-equivalent scope:
//   - net.bisks.elopt.optin (create+update+delete — opting in writes it to a
//     fixed "self" rkey, opting out deletes it)
//   - net.bisks.elopt.vote (create+update — a battle vote, deterministic
//     rkey so re-voting the same matchup overwrites instead of stacking)
...
const ORIGIN = location.origin; // https://elopt.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is elopt.bisks.net — no path mount
...
const SCOPE =
  "atproto repo:net.bisks.elopt.optin?action=create&action=update&action=delete repo:net.bisks.elopt.vote?action=create&action=update";

// oauth.js — browser-side atproto OAuth (public client), for mootfluence.bisks.net.
//
// Copied and trimmed from sites/influential25/public/lib/oauth.js (same repo,
// same pattern). Public client:
...
//
// The client_id is the URL of our static client-metadata.json. Sign-in is
// only needed for the "make a starter pack" step — viewing a ranking is
// anonymous, public-AppView reads. Scope is narrowed to exactly the three
// collections a starter pack write touches, create-only — no account-wide
// access, no update/delete. Must stay in lockstep with client-metadata.json.
...
const ORIGIN = location.origin; // https://mootfluence.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is mootfluence.bisks.net — brand new site, no legacy path route
...
export const SCOPE =
  "atproto repo:app.bsky.graph.list?action=create repo:app.bsky.graph.listitem?action=create repo:app.bsky.graph.starterpack?action=create";
...
const DB_NAME = "mootfluence-oauth";
const STORE = "session";
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever query string the user started the login from (a
  // handle already typed in, say), not the bare redirect_uri the auth server
  // sent us to. The session is already in IndexedDB, so the fresh load picks
  // it straight up via getSession(), no re-auth.

// twin.js — simcluster-twin's "advanced algorithm": pulls recent posts for a
// handle and everyone in its SimCluster pool, turns each account's post
// history into a small feature bundle, and scores every pairwise match
// against you on four axes — vocabulary overlap (cosine similarity over word
// frequency), hashtag overlap (Jaccard), posting-style distance (avg length,
// reply ratio, emoji rate), and time-of-day rhythm (cosine over a 24-bucket
// UTC-hour histogram). Weighted average of whichever axes have data → one
// 0–100 "compatibility" number per pool member. Highest is your twin, lowest
// is your anti-twin. It's real math, just not a very deep model — see the
// "how this works" copy in index.html, we're not pretending otherwise.
//
// Everything reads Bluesky's public AppView anonymously (CORS *). No network
// calls in this file except fetchPosts; feature extraction and scoring are
// pure functions over data the caller already has.

const API = "https://public.api.bsky.app/xrpc/";
const PAGES_PER_ACCOUNT = 2; // up to 200 posts per account
export const MIN_POSTS = 5; // below this, an account is skipped as too sparse to score
...
export async function fetchPosts(did, { onStep, pages = PAGES_PER_ACCOUNT } = {}) {
  ...
  for (let page = 0; page < pages; page++) {
    onStep && onStep(`(${out.length} posts so far)…`);
...
// A short stoplist skewed toward how people actually post (contractions,
// filler, atproto-flavored fillers) rather than a formal-English list.
...
// Scores account `b` against `a` (you). Returns { score, breakdown, shared,
// unique } where breakdown holds each axis's 0–1 similarity (or null if that
// axis had no data on one side and was excluded from the average).
export function compare(a, b) {
  const wordSim = cosine(a.wordFreq, b.wordFreq);
  const hashtagSim = jaccard(a.hashtagFreq, b.hashtagFreq);
  const timeSim = cosineArr(a.hourHist, b.hourHist);

  const lengthDiff = Math.min(1, Math.abs(a.avgLength - b.avgLength) / 300);
  const replyDiff = Math.abs(a.replyRatio - b.replyRatio);
  const emojiDiff = Math.min(1, Math.abs(a.emojiRate - b.emojiRate) / 3);
  ...
  const sharedWords = [...a.wordFreq.keys()]
    .filter((w) => b.wordFreq.has(w))
    .sort((x, y) => (b.wordFreq.get(y) + a.wordFreq.get(y)) - (b.wordFreq.get(x) + a.wordFreq.get(x)))
    .slice(0, 8);
  const onlyYou = a.topWords.filter((w) => !b.wordFreq.has(w)).slice(0, 6);
  const onlyThem = b.topWords.filter((w) => !a.wordFreq.has(w)).slice(0, 6);
  const sharedTags = [...a.hashtagFreq.keys()].filter((t) => b.hashtagFreq.has(t)).slice(0, 6);

  return {
    score,
    axes: axes.map((x) => ({ key: x.key, value: x.value })),
    sharedWords,
    onlyYou,
    onlyThem,
    sharedTags,

...
TOOLS.forEach(([label, color], i) => {
  const x = 60 + i * 122;
  const y = H - 66;
  toolRow += `<rect x="${x}" y="${y - 24}" width="104" height="48" rx="10" fill="#241621" stroke="#402a38" stroke-width="2"/>
    <text x="${x + 52}" y="${y + 7}" text-anchor="middle" font-family="JetBrains Mono" font-weight="700" font-size="20" fill="${color}">${label}</text>`;
});

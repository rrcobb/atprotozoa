// global-index.js — network-wide view over the witness collection:
...
// the entirety of witness's "backend" — there is no server-side index at
// all, so the feed and the duplicate-report tally are only ever a replay of
// whatever's really sitting on everyone's own PDS.
// Copied verbatim from sites/shelfguessr/public/lib/global-index.js (itself
// from sites/skymash / sites/rateyourbuild / sites/steamtags — see
...
// this one instance indexes net.bisks.witness.entry.
//
...
export class GlobalIndex {
  ...
  constructor(collection, { normalize, onUpdate } = {}) {
    ...
    this.cacheKey = `witness:global-index:${collection}:v2`;
  ...
  emit() {
    try {
      ...
      console.error("witness global-index render failed", err);
    }
  }
  ...
  async runBackfill() {
    ...
    try {
      ...
      retryMs = 5000;
      console.warn("witness global-index backfill failed", err);
    } finally {
      this.backfillRunning = false;
      this.scheduleEmit();

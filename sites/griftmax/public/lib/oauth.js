// oauth.js — browser-side atproto OAuth (public client), for griftmax.bisks.net.
//
...
// narrow create-only `repo:` grant on a single custom collection — not the
// old `transition:generic` app-password-equivalent scope, which would hand
// this joke site read/write over the signer's *entire* repo for a button
// that only ever writes one small record. "Ascending" writes a
// net.bisks.griftmax.ascension record to the *acting user's own* PDS; the
// Worker never trusts a client-supplied identity, it reads the record back
// off the claimed author's own PDS to confirm it's real (verifyOwnRecord in
// src/index.ts, same pattern as hyperobject's Pit).
//
// The CAR export ("audit your bag") needs no scope at all —
// com.atproto.sync.getRepo is an unauthenticated public read on the user's
// own PDS, same as any other repo backup tool.
...
const ORIGIN = location.origin; // https://griftmax.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is griftmax.bisks.net — no path mount
...
const SCOPE = "atproto repo:net.bisks.griftmax.ascension?action=create";
...
export async function login(handleOrDid) {
  ...
  // established, so signing in from any page takes you back to that page.

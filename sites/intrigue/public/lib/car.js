...
export async function fetchRepoRecords(pds, did, types, onProgress, options = {}) {
  ...
  const res = await fetch(pds.replace(/\/$/, "") + "/xrpc/com.atproto.sync.getRepo?did=" + encodeURIComponent(did), { signal: options.signal });
  if (!res.ok) {
  ...
  for (const blockBytes of carBlocks(bytes)) {
    if (options.signal && options.signal.aborted) throw new DOMException("scan cancelled", "AbortError");
    scanned++;
    if (scanned > CAR_MAX_BLOCKS) {
      if (onProgress) onProgress(`scan limit reached at ${CAR_MAX_BLOCKS.toLocaleString()} CAR blocks`);
      throw new Error("repo CAR exceeds the browser scan limit");
    }
    let obj;
    try { obj = cborDecode(blockBytes); } catch (_) { continue; }
    if (!obj || typeof obj !== "object" || Array.isArray(obj) || !wanted.has(obj.$type)) continue;
    out.push(obj);
    if (onProgress && scanned % 500 === 0) onProgress(`scanning repo CAR... ${scanned.toLocaleString()} blocks, ${out.length.toLocaleString()} matching records`);
  }

// oauth.js — browser-side atproto OAuth (public client), for bsky95.bisks.net.
//
...
//
// bsky95 is a dedicated subdomain (not path-mounted), so client_id/redirect_uri
// are just location.origin with no prefix.
//
// Scope is narrowed to exactly what bsky95 touches: create-only on
// app.bsky.feed.post (New Post / Reply from the compose window), create+delete
// on app.bsky.feed.like and app.bsky.feed.repost (the per-post web/fly icons in
// a feed window), plus rpc grants for the four calls proxied through the
// user's own PDS to the AppView (personal timeline, notifications list/seen/
// unread-count). Everything else bsky95 reads (the public Discover feed,
// search, trending topics) goes through the public unauthenticated AppView
// and needs no scope at all. See notes/50-oauth-scopes.md.
...
const ORIGIN = location.origin; // https://bsky95.bisks.net (or localhost in dev)
...
const SCOPE =
  "atproto repo:app.bsky.feed.post?action=create repo:app.bsky.feed.like?action=create&action=delete repo:app.bsky.feed.repost?action=create&action=delete rpc:app.bsky.feed.getTimeline?aud=did:web:api.bsky.app%23bsky_appview rpc:app.bsky.notification.listNotifications?aud=did:web:api.bsky.app%23bsky_appview rpc:app.bsky.notification.updateSeen?aud=did:web:api.bsky.app%23bsky_appview rpc:app.bsky.notification.getUnreadCount?aud=did:web:api.bsky.app%23bsky_appview";

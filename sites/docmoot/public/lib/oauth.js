// oauth.js — browser-side atproto OAuth (public client), for bisks.net/docmoot.
//
// Copied and trimmed from padmoot/public/lib/oauth.js (same repo). Public
// client:
...
// narrowed to create-only on net.bisks.docmoot.snapshot — no account-wide
// access. Must stay in lockstep with client-metadata.json. Only used for the
// "save snapshot to my
// PDS" button — editing a doc itself needs no login.
...
// Canonical host is docmoot.bisks.net, so there is no mount prefix. The site is
// still reachable at bisks.net/docmoot, but OAuth only works on the canonical
// host: client_id must equal the URL the metadata is actually served from.
export const MOUNT = ""; // canonical host is docmoot.bisks.net — see audit/migrate-oauth-hosts.mjs
...
const SCOPE = "atproto repo:net.bisks.docmoot.snapshot?action=create";
...
export async function login(handleOrDid) {
  ...
  // the auth server's callback can't carry along whatever ?owner=&rkey= the
  // user was looking at when they clicked "sign in" — e.g. a shared board
  // link. We stash that here as `returnTo` and completeLoginIfCallback()
  // navigates back to it once the session is established, so signing in from
  // a board takes you back to that same board instead of dumping you on /.

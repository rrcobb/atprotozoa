// oauth.js — browser-side atproto OAuth (public client), for mutechella.bisks.net.
//
// Forked from sites/blocknotes/public/lib/oauth.js (same repo), trimmed to a
...
//
// mutechella never writes anything — it signs you in only to read your own
// mute list (app.bsky.graph.getMutes is account-side AppView state gated to
// the account owner, not public data), so the scope is exactly that one rpc
// grant. No repo: grants, no blob: grants.
//
...
const ORIGIN = location.origin; // https://mutechella.bisks.net (or localhost in dev)
const MOUNT = ""; // canonical host is mutechella.bisks.net, served at the root
...
const SCOPE =
  "atproto rpc:app.bsky.graph.getMutes?aud=did:web:api.bsky.app%23bsky_appview";

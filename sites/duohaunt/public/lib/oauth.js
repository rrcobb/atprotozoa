// oauth.js — browser-side atproto OAuth (public client), for duohaunt.bisks.net.
//
// Copied and trimmed from sites/hyperobject/public/lib/oauth.js (same repo),
// itself from clusterpedia/padmoot, adapted from trigrams/public/lib/oauth.js /
...
// narrow grants: `repo:net.bisks.duohaunt.checkin?action=create` (writes a
// tiny record to your own PDS every time your overdue count changes — the
// Worker reads it back to verify it before it counts, see src/index.ts's
// verifyOwnRecord) and `repo:app.bsky.feed.post?action=create` (the "confess
// publicly" button, always a live create from your own signed-in session —
// this app never stores your token past the request that needs it, see the
// big comment at the top of src/index.ts for why). Not the old
// `transition:generic` app-password-equivalent scope, which would hand this
// app read/write over your *entire* repo for a feature that only ever
// creates two kinds of record.
...
const ORIGIN = location.origin; // https://duohaunt.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is duohaunt.bisks.net — no path mount
...
const SCOPE =
  "atproto repo:net.bisks.duohaunt.checkin?action=create repo:app.bsky.feed.post?action=create";

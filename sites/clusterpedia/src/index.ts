...
export class WikiStore {
  ...
  // and the resulting article: entry — but only if `slug` doesn't already
  // exist. `revisions` must be given oldest-first; the last one becomes the
  // article's current text.
  private async writeArticleRevisions(
    ...
    if (await this.storage.get(artKey, "json")) return;

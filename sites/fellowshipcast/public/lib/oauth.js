// oauth.js — browser-side atproto OAuth (public client), for
// fellowshipcast.bisks.net.
//
// Copied and adapted from sites/catspace/public/lib/oauth.js (same repo),
// descended from sites/goodsky/public/lib/oauth.js / mino.mobi's
...
//
// Scope: narrowed per notes/50-oauth-scopes.md — fellowshipcast only ever
// creates or updates its own net.bisks.fellowshipcast.ballot "self" record
// (one ballot per voter; revoting overwrites it in place). Must match
...
const ORIGIN = location.origin; // https://fellowshipcast.bisks.net (or localhost in dev)
const MOUNT = "";
...
const SCOPE = "atproto repo:net.bisks.fellowshipcast.ballot?action=create&action=update";
...
const DB_NAME = "fellowshipcast-oauth";
const STORE = "session";
...
export async function completeLoginIfCallback() {
  ...
  // user started the flow from a non-root page a plain history.replaceState
  // would leave the already-loaded root document's DOM sitting under the
  // wrong URL — a real navigation is needed so the browser actually fetches
  // that page's HTML.

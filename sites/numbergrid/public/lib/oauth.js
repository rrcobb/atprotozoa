// oauth.js — browser-side atproto OAuth (public client), for numbergrid.bisks.net.
//
...
// narrow create-only `repo:` grant on net.bisks.numbergrid.number — this site
// only ever appends number records to the signed-in user's own repo, never
// reads or writes anything else, so `transition:generic` (full account
// access) would be far broader than what's actually used.
...
const ORIGIN = location.origin; // https://numbergrid.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is numbergrid.bisks.net — no path mount
...
const SCOPE = "atproto repo:net.bisks.numbergrid.number?action=create";
...
const DB_NAME = "numbergrid-oauth";
const STORE = "session";

// oauth.js — browser-side atproto OAuth (public client), for relegation.bisks.net.
//
// Copied and trimmed from sites/vouch/public/lib/oauth.js (same repo),
// itself descended from hyperobject/clusterpedia/padmoot/trigrams/mino.mobi's
...
//
// The client_id is the URL of our static client-metadata.json. Scope is the
// bare `atproto` grant — login only, no repo/rpc/blob permissions — because
// this site never writes anything and never proxies a call through the
// signed-in user's PDS: every read (own repo CAR, likes, replies, follows,
// followers) is public AppView/PDS data fetched unauthenticated. Login exists
// solely to know *whose* account to analyze without asking them to type their
// own handle in.
...
const ORIGIN = location.origin; // https://relegation.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is relegation.bisks.net — no path mount
...
// DID -> DID doc, via plc.directory or did:web's well-known document.
...
// DID -> handle, via the DID doc's alsoKnownAs (`at://<handle>`). Falls back
// to the DID string if nothing's found.
...
const DB_NAME = "relegation-oauth";
const STORE = "session";
...
export async function login(handleOrDid) {
  ...
  // Stash what the callback needs. redirect_uri is a fixed "/" (must match
  // client-metadata.json exactly), so we stash `returnTo` here and
...
// Make a DPoP-authenticated request to the user's PDS. Refreshes the access
// token when it's expired (or when the server says invalid_token), handles
// the use_dpop_nonce challenge, and persists the rotated nonce.

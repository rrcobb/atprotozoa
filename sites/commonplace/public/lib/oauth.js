// oauth.js — browser-side atproto OAuth (public client), for commonplace.bisks.net.
//
// Copied and trimmed from sites/padmoot/public/lib/oauth.js (same repo),
// itself adapted from trigrams/public/lib/oauth.js / mino.mobi's
...
// narrowed to create-only on site.standard.publication / .document plus
// image blob uploads — no account-wide access. Must stay in lockstep with
...
const ORIGIN = location.origin; // https://commonplace.bisks.net (or localhost in dev)
// commonplace has its own hostname and no path-route legacy, so there is no
// mount prefix: client_id must equal the URL the metadata is actually served
// from.
export const MOUNT = "";
...
const SCOPE =
  "atproto repo:site.standard.publication?action=create repo:site.standard.document?action=create blob:image/*";
...
const DB_NAME = "commonplace-oauth";
const STORE = "session";
...
export async function login(handleOrDid) {
  ...
  // the auth server's callback can't carry along whatever ?read=<did>/<rkey>
  // the user was looking at when they clicked "sign in" — e.g. a shared
  // reading link. We stash that here as `returnTo` and
  // completeLoginIfCallback() navigates back to it once the session is
  // established, so signing in from a shared link lands back on that link.
...
export async function completeLoginIfCallback() {
  ...
  // bare redirect_uri MOUNT + "/" the auth server sent us to.

...
// one page at a time. One request gets every app.bsky.graph.block AND
// app.bsky.graph.follow record an account has ever written, in one shot — no
// per-page cap, so a heavily-blocking or heavily-following account's lists
// don't get silently truncated (see notes/40-new-site-playbook.md's cee.wtf
...
// sites/kevinmoot's lib/car.js (itself via sites/backscroll/activitygrid/
// ngmi/beefcheck) — listrank only needs each record's `subject` DID, not the
// rkey, but fetchRepoRecordsWithKeys returns both so the same file works
// unmodified.
//

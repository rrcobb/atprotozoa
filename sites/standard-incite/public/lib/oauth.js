...
// standard-incite.bisks.net. The only thing a session is ever used for is
// writing a net.bisks.standard-incite.optout "self" record to the signed-in
// account's own PDS — searching a handle's moots never needs a login.
//
// Copied and trimmed from sites/skymash/public/lib/oauth.js (same repo).
// Public client:
...
// narrowed to net.bisks.standard-incite.optout, create+update only (it's
// "write, or overwrite in place", never a delete) — no account-wide access.
...
const ORIGIN = location.origin; // https://standard-incite.bisks.net (or localhost in dev)
export const MOUNT = ""; // own subdomain from day one — no path mount
...
const SCOPE = "atproto repo:net.bisks.standard-incite.optout?action=create&action=update";
...
const DB_NAME = "standard-incite-oauth";
const STORE = "session";
...
export async function login(handleOrDid) {
  ...
  // the auth server's callback can't carry along whatever the user was doing
  // when they clicked "sign in". We stash that here as `returnTo` and
...
export async function completeLoginIfCallback() {
  ...
  // Land back on wherever the user started the login from, not the bare
  // redirect_uri MOUNT + "/" the auth server sent us to.

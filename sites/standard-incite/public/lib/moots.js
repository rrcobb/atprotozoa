...
// followers). Reads Bluesky's public AppView anonymously (public.api.bsky.app,
// CORS *, no auth): resolveHandle, getFollows, getFollowers, getProfile.
// Trimmed from sites/dial-a-mutual/public/lib/moots.js (copy, don't abstract)
// — dropped latestPost() (this site cares about site.standard.document posts,
// not Bluesky posts) and the 200-slot speed-dial cap: standard incite's whole
...
// budget — raised to 400 across the moot-family sites on 2026-08-28 (see
// CLAUDE.md's "Where the no-caps rule came from"). Every follow/follower is
// still walked; this just bounds how far a getFollows/getFollowers loop can
// run before giving up on an unreachable cursor.
...
// `pool` excludes self and holds every mutual — no truncation.
export async function moots(actor, { onStep } = {}) {
  ...
  return { did, handle: self.handle, self, pool };
}

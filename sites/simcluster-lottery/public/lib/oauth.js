// oauth.js — browser-side atproto OAuth (public client), for simcluster-lottery.bisks.net.
//
// Copied from sites/nothingness/public/lib/oauth.js (itself from
// sites/trigrams/public/lib/oauth.js, adapted from mino.mobi's
// airchat/oauth/flow.js, minormobius/agent01, thank you), trimmed for a
// login-only site: the lottery never writes to the user's PDS — wallet
// balance, tickets, and the leaderboard all live in this site's own Durable
// Object, keyed by DID. So scope is the bare `atproto` (no `transition:generic`,
// no `repo:` grant) and the redirect lands back on `/`. Public client:
...
const ORIGIN = location.origin; // https://simcluster-lottery.bisks.net (or localhost in dev)
...
const DB_NAME = "simcluster-lottery-oauth";
const STORE = "session";

...
// exactly the two grants this site's write surface needs — create a post
// (app.bsky.feed.post) and upload the image it optionally embeds — not the
// old `transition:generic` app-password-equivalent scope, which would hand
// us read/write over the signer's *entire* repo. See public/lib/post.js for
// the createRecord/uploadBlob calls this scope covers.
...
const ORIGIN = location.origin; // https://simcluster-shadow.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is simcluster-shadow.bisks.net — no path mount
...
const DB_NAME = "simcluster-shadow-oauth";
const STORE = "session";

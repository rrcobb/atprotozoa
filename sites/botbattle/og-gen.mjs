// Generates public/og.png — the Open Graph preview card for bot battle.
...
// loaded explicitly). Copied and trimmed from sites/alice-meets-bob/og-gen.mjs.
//
...
const BG = "#0b0710", BG2 = "#1a0e14", FG = "#f4ecf5", DIM = "#a291ab";
const A = "#ff4d6d", B = "#4da8ff", GOLD = "#ffd166", CARD = "#16101c", BORDER = "#33253a";
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <!-- two facing fighter silhouettes either side of a crossed-swords VS -->
  ...
  <text x="${midX}" y="${cardY + 320}" text-anchor="middle" font-family="JetBrains Mono" font-weight="700" font-size="58" fill="${FG}">bot battle</text>
  <text x="${midX}" y="${cardY + 372}" text-anchor="middle" font-family="JetBrains Mono" font-size="23" fill="${DIM}">any two atproto accounts enter — real stats decide it</text>

  <g font-family="JetBrains Mono" font-size="19" fill="${DIM}">
    <text x="${midX}" y="${cardY + 420}" text-anchor="middle">except buildthis.bisks.net, which just wins. every time.</text>
  </g>

  <text x="${midX}" y="${cardY + cardH - 34}" text-anchor="middle" font-family="JetBrains Mono" font-size="20" fill="${GOLD}">bisks.net/botbattle</text>
</svg>`;

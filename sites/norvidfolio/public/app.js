// norvidfolio — glue script. Profile header + image gallery are live (public
// AppView, anonymous, via lib/feed.js — copied from sites/meadowfolio, which
// copied it from sites/portfolio). Curator picks are static, from ./data.js.
...
// ── curator picks ────────────────────────────────────────────────────────
// Some picks (the highest-liked one, in fact) have no caption text at all —
// the blockquote is skipped rather than rendered empty for those.
function renderPicks() {
  ...
  el.innerHTML = PICKS.map((p) => {
    ...
    const quote = p.text
      ...
      : `<blockquote class="empty"><a href="${permalink(p.rkey)}" target="_blank" rel="noopener">(no caption)</a></blockquote>`;
    return `
      <figure class="pick">
        ${img}
        ${quote}

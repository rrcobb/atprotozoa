// mutualsky Worker — served at the root of mutualsky.bisks.net
//
...
//    Bluesky's own link-unfurl cache would show one generic "mutualsky" card
...
const SITE_ORIGIN = "https://mutualsky.bisks.net";
...
async function renderPost(env: Env, request: Request, handle: string, rkey: string): Promise<Response> {
  ...
  const name = profile.displayName ? `${profile.displayName} (@${profile.handle})` : `@${profile.handle}`;
  const text = post?.record?.text ? truncate(post.record.text, 220) : `A post by ${name} on mutualsky.`;
  const image =

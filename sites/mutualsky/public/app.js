...
const MOUNT = ""; // canonical host is mutualsky.bisks.net, served at the root
const SITE_URL = "https://mutualsky.bisks.net/";
...
// ---------- auth (OAuth login -> mutuals-only home timeline) ----------
//
// Forked from sites/skyclone. mutualsky is still a read-only viewer for
// anyone who doesn't log in — this adds an optional real OAuth session (see
// lib/oauth.js) so a logged-in visitor sees their actual home timeline
// (app.bsky.feed.getTimeline, proxied through their own PDS), same as
// skyclone. The one real difference: loadTimeline (below) filters that feed
// down to posts from your mutuals only — people who follow you back — instead
// of showing everyone you follow. See getMutualDids/loadTimeline for the
// filter itself.
...
// session. mutualsky reads through the public (unauthenticated) AppView, which
...
function openLoginModal() {
  ...
  box.innerHTML = `
    <div class="modal">
      ...
      <p>Real OAuth, straight to your own PDS — mutualsky never sees your password. This unlocks your real home timeline (filtered to your mutuals — see the About page), and lets you catch posts in your web, repost, and reply — all genuine writes to your own repo. It still never follows anyone on your behalf.</p>
...
function postCard(post, opts) {
  ...
  const shareText = `Post by @${author.handle} on mutualsky: "${(record.text || "").slice(0, 120)}"`;
...
function shellHtml(activePath) {
  ...
  return `
  <div class="mobile-topbar"><span>🕷️ mutualsky</span>${mobileAuthHtml()}</div>
  <div class="shell">
    <nav class="nav">
      <a class="nav-logo" href="${MOUNT}/" data-link><span class="wing">🕷️</span><span class="word">mutualsky</span></a>
...
function asideHtml() {
  ...
  <div class="aside-search">
    <span>🔎</span>
    <input id="aside-q" placeholder="Search mutualsky" autocomplete="off">
  </div>
  <div class="aside-card">
    <h2>What is this?</h2>
    <p>mutualsky is a fan-made rebuild of the bsky.app web client. Every feed, profile, and thread here is live data pulled straight from Bluesky's public AppView — nothing is faked or cached long-term.</p>
    <p>Browse without an account, or log in with OAuth to see your real home timeline, spin your own posts, catch posts in your web (a real like, no hearts), repost, and reply — genuine writes to your own repo. mutualsky never follows for you. Real notifications live in the 🔔 tab — a spider crawls across your screen when a new one comes in. For DMs, use <a class="link" href="https://bsky.app" target="_blank" rel="noopener">bsky.app</a>.</p>
  </div>
...
async function HomeView(main, params) {
  let tabs = session ? [{ label: "Mutuals", uri: TIMELINE_URI }] : [];
...
// ---------- mutuals (the whole point of this fork) ----------
//
// A "mutual" is someone who follows you and you follow back — i.e. the
// intersection of getFollows(you) and getFollowers(you). Both are public,
// unauthenticated AppView reads (same xrpc() used for everything else), so
// this needs no extra OAuth scope beyond knowing session.did. Cached per DID
// for the tab's lifetime so switching feeds/paging the timeline doesn't
...
async function getMutualDids(did) {
  if (!mutualsCache.has(did)) {
    mutualsCache.set(
      did,
      Promise.all([
        collectActorDids("app.bsky.graph.getFollows", did),
        collectActorDids("app.bsky.graph.getFollowers", did),
      ]).then(([follows, followers]) => new Set([...follows].filter((d) => followers.has(d))))
    );
  }
  return mutualsCache.get(did);
}

// getTimeline for a logged-in visitor — proxied through their own PDS to the
// AppView (same DPoP-bound session lib/oauth.js sets up) — filtered down to
// posts authored by a mutual (or by you): the "shortcut to a breakthrough"
// this whole fork exists for, instead of skyclone's unfiltered getTimeline.
// getTimeline pages mostly contain non-mutuals (everyone you follow, not just
// follow-backs), so a single page rarely fills the box — this walks forward
// through pages itself, accumulating filtered posts, up to a small fetch cap
// per call so "Load more" can't spin forever on a quiet page.
const TIMELINE_FETCH_CAP = 8;

async function loadTimeline(box, cursor) {
  if (!session) {
    box.innerHTML = centerMsg("Not signed in", "Log in with Bluesky (top of the nav) to see your mutuals-only timeline.");
    return;
  }
  try {
    const mutuals = await getMutualDids(session.did);
    const { dpopFetch } = await oauth();
    const collected = [];
    ...
    do {
      ...
      for (const item of data.feed || []) {
        ...
        if (authorDid === session.did || mutuals.has(authorDid)) collected.push(item);
      }
      nextCursor = data.cursor;
    } while (nextCursor && collected.length < 25 && fetches < TIMELINE_FETCH_CAP);

    if (!cursor) box.innerHTML = "";
    else box.querySelector(".load-more")?.remove();
    box.insertAdjacentHTML(
      "beforeend",
      collected.map(feedItemHtml).join("") ||
        centerMsg("Nothing here yet", "No mutuals posting right now — follow some people who follow you back, or check again later.")
    );
    if (nextCursor) {
      box.insertAdjacentHTML("beforeend", loadMoreBtn());
      box.querySelector(".load-more").onclick = (e) => {
        e.target.textContent = "Loading…";
        loadTimeline(box, nextCursor);
      };
    }
  } catch (e) {
    box.innerHTML = errorBox("Couldn't load your mutuals timeline (" + e.message + ")", location.pathname + location.search);
  }
}
...
async function ProfileView(main, params, args) {
  ...
  const shareText = `${profile.displayName || profile.handle} (@${profile.handle}) on mutualsky`;
...
async function ThreadView(main, params, args) {
  ...
  try {
    ...
    const shareText = `${focus.author.displayName || focus.author.handle} (@${focus.author.handle}) on mutualsky: "${(focus.record.text || "").slice(0, 100)}"`;
...
function AboutView(main) {
  main.innerHTML =
    headerHtml("About mutualsky") +
    `<div style="padding:16px;font-size:15px;line-height:1.6">
      <p><b>mutualsky</b> is an unofficial rebuild of the bsky.app web client — the home feed, profiles, threads, feed discovery, search, and notifications, all wired to Bluesky's live public AppView (<code>public.api.bsky.app</code>) instead of a database of its own. It's a fork of <a class="rt-link" href="https://skyclone.bisks.net" target="_blank" rel="noopener">skyclone</a>, atprotozoa's unfiltered bsky.app clone, with one change: the "shortcut to a breakthrough" a mutual suggested when someone asked the bot to build a client that only shows Good Posts — skip the hard part, and just show mutuals.</p>
      <p>The <b>Mutuals</b> tab is <code>app.bsky.feed.getTimeline</code> (your real algorithmic timeline, proxied through your own PDS), filtered client-side to posts from people who follow you back — the intersection of your follows and your followers, computed live from the public AppView. No account is required to browse; logging in is optional and uses real atproto OAuth (PKCE + DPoP) straight to your own PDS — mutualsky never sees your password. Every byte you see (posts, likes, follower counts, avatars) is fetched fresh from Bluesky at request time; nothing is stored server-side. Interactions are real writes to your own repo, straight to your PDS, no AppView proxy: catching a post in your web is a genuine <code>app.bsky.feed.like</code> (drawn as a spider, not a heart), 🪰 is a genuine <code>app.bsky.feed.repost</code> (a fly, loosed back into the web), and 💬 opens a compose box that writes a genuine <code>app.bsky.feed.post</code> with a real reply ref. mutualsky still never follows anyone or touches your DMs for you.</p>
      <p>Notifications (🔔) are real too — a genuine <code>app.bsky.notification.listNotifications</code> call through your own PDS. mutualsky quietly polls for new ones in the background, and when one lands, a spider crawls across your screen. AHH!</p>
      <p>Every "Liked by" list also has a 🕸️ next to each name — a like's subject can be any record per the lexicon, including someone else's like, so mutualsky lets you like a like (a real write) and see who's meta-liked it. bsky.app never built this UI; it isn't hiding, it's just not exposed. Keep clicking through and it's meta-likes all the way down.</p>
...
function NotFoundView(main) {
  main.innerHTML = headerHtml("Not found") + centerMsg("Nothing here", "That page doesn't exist in mutualsky.");
}

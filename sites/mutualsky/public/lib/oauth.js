// oauth.js — browser-side atproto OAuth (public client), for mutualsky.bisks.net.
//
// Forked from sites/skyclone/public/lib/oauth.js (same repo), itself
// descended from sites/drivethru/public/lib/oauth.js / slop-shop /
// mino.mobi's airchat/oauth/flow.js. Public client:
...
//
// Scope is narrowed to exactly what mutualsky touches — identical surface to
// skyclone, since this is the same client with a filtered home feed, not a
// reduced one: create-only on app.bsky.feed.post (posts/replies),
// create+delete on app.bsky.feed.like, app.bsky.feed.repost, and
// app.bsky.graph.block, plus rpc grants for the four calls proxied through
// the user's own PDS to the AppView (personal timeline, notifications
// list/seen/unread-count). The mutuals filter itself (getFollows/getFollowers
// on your own DID) reads through the public unauthenticated AppView and needs
// no scope at all. Everything else read (profiles, threads, search, other
// people's feeds) is the same public, scope-free path.
//

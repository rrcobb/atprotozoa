// oauth.js — browser-side atproto OAuth (public client), for nothingness.bisks.net.
//
// Copied from sites/trigrams/public/lib/oauth.js (itself adapted from mino.mobi's
// airchat/oauth/flow.js, minormobius/agent01, thank you), trimmed for a
// login-only site: nothingness never writes to the user's PDS, it just needs
// their DID to attribute leaderboard clicks — so scope is the bare `atproto`
// (no `transition:generic`) and the redirect lands back on `/`, not a
// dedicated launcher route. Public client:
...
const ORIGIN = location.origin; // https://nothingness.bisks.net (or localhost in dev)
...
const DB_NAME = "nothingness-oauth";
const STORE = "session";

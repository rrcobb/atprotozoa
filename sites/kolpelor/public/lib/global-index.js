// global-index.js — the /atlas page's data source: every
// net.bisks.kolpelor.roster record on the network, found via
// listReposByCollection and kept live via Jetstream. No server, no Durable
// Object — a directory of records that already live in everyone's own PDS
// doesn't need a copy sitting in a database. Copied and trimmed from
// sites/catspace/public/lib/global-index.js — same backfill+live pattern for
// a singleton "self" record (see net.bisks.kolpelor.roster.json's `key`).
//
// This is the network-wide counterpart to app.js's Ἴχνη (tracks) scan, which
// only checks your own SimCluster's moots via a targeted getRecord each. The
// Atlas has no such scope — every trainer who's ever signed in and synced a
// roster shows up here, moot or stranger.

const COLLECTION = "net.bisks.kolpelor.roster";
...
const CACHE_KEY = "kolpelor:global-index:v1";
...
function avatarUrl(pdsUrl, did, party) {
  const first = Array.isArray(party) ? party[0] : null;
  return first && typeof first.avatar === "string" ? first.avatar : null;
}
...
function normaliseRecord(did, record, identity) {
  ...
  return {
    ...
    partyAvatar: avatarUrl(identity.pdsUrl, did, party),
    wins: Number(record.wins) || 0,
    losses: Number(record.losses) || 0,
    ladderRank: Number(record.ladderRank) || 0,
    aristos: !!record.aristos,
...
export class GlobalIndex {
  ...
  snapshot() {
    const trainers = Array.from(this.entries.values()).sort((a, b) => {
      if (a.aristos !== b.aristos) return a.aristos ? -1 : 1;
      if (b.wins !== a.wins) return b.wins - a.wins;
      return b.updatedAt - a.updatedAt;
    });
    return {
      updatedAt: this.lastUpdated || null,
      trainerCount: this.entries.size,
  ...
  emit() {
    try {
      ...
      console.error("kolpelor global render failed", err);
    }
  }
  ...
  async runBackfill() {
    ...
    try {
      ...
      retryMs = 5000;
      console.warn("kolpelor global backfill failed", err);
    } finally {
      this.backfillRunning = false;
      this.scheduleEmit();

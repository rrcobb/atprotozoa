// oauth.js — browser-side atproto OAuth (public client), for blubberize.bisks.net.
//
// Copied and trimmed from sites/hyperobject/public/lib/oauth.js (same repo),
// itself from sites/velvetrope, clusterpedia/padmoot, trigrams, mino.mobi's
...
// narrow create-only `repo:` grant on net.bisks.blubberize.fission — not the
// old `transition:generic` app-password-equivalent scope, which would hand us
// read/write over the signer's *entire* repo. Signing in only ever does one
// thing: write a fission receipt to the signed-in user's own PDS when they
// hit FISSION (see app.js's doFission). Nothing here ever writes on anyone
// else's behalf, and nothing about a fission touches the target's real
// ability to post — the record is a receipt, not an action against them.
...
const ORIGIN = location.origin; // https://blubberize.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is blubberize.bisks.net — no path mount
...
const SCOPE = "atproto repo:net.bisks.blubberize.fission?action=create";
...
const DB_NAME = "blubberize-oauth";
const STORE = "session";
...
export async function login(handleOrDid) {
  ...
  // established, so signing in from a shared /s/ link takes you back to it.
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page (a shared /s/ link, ?you=&target=, ...) the
  // user started the login from, not the bare redirect_uri MOUNT + "/" the
  // auth server sent us to.

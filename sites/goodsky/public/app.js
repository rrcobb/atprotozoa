...
const MOUNT = ""; // canonical host is goodsky.bisks.net
const SITE_URL = "https://goodsky.bisks.net/";
...
//
// goodsky is still a read-only viewer for anyone who doesn't log in — this
...
// Nothing here ever writes to the user's repo except the actions you take.
...
// session. goodsky reads through the public (unauthenticated) AppView, which
...
async function goodposts() {
  if (!goodpostsLib) goodpostsLib = await import(`${MOUNT}/lib/goodposts.js`);
  return goodpostsLib;
}
...
function openLoginModal() {
  ...
  box.innerHTML = `
    <div class="modal">
      ...
      <p>Real OAuth, straight to your own PDS — goodsky never sees your password. This unlocks your actual home timeline (filtered, same as everything else here), and lets you like, repost, and reply — all genuine writes to your own repo. It still never follows anyone on your behalf.</p>
...
// Likes and reposts are optimistic UI, reverted on failure.
...
// Manual per-post hide — cosmetic only. No repo write, nothing deleted; the
// post itself is untouched, this tab just stops showing it. A backstop for
// anything the goodposts filter (lib/goodposts.js) let through that you'd
// rather not see.
function hidePost(actionEl) {
  const target = actionEl.closest(".post, .thread-focus");
  if (!target || target.classList.contains("hiding")) return;
  ...
  requestAnimationFrame(() => target.classList.add("hide-collapse"));
}
  ...
  box.innerHTML = `
    <div class="modal">
      ...
      <div class="modal-actions">
        ...
        <button type="button" id="compose-go" class="pill-btn primary">Post</button>
      </div>
...
function likeActionHtml(post, url) {
  ...
  return `<span class="act like${liked ? " liked" : ""}">
    <span class="like-icon" data-action="like" data-uri="${esc(post.uri)}" data-cid="${esc(post.cid || "")}" title="${liked ? "Caught in your web" : "Catch it in your web"}">${liked ? "❤️" : "🤍"}</span>
...
// Not a real write — nothing is deleted from anyone's repo, this just
// collapses the post out of *your* view. Refresh (or another visitor) and
// it's right back. A manual backstop alongside the goodposts filter above.
function hideActionHtml() {
  return `<span class="act hide" data-action="hide" title="Hide this post">✕</span>`;
}
...
function postCard(post, opts) {
  ...
  const shareText = `Post by @${author.handle} on goodsky: "${(record.text || "").slice(0, 120)}"`;
...
// ---------- goodposts filter bar (the whole point of the site) ----------
//
// A small control strip shown above any feed of posts (home, profile, custom
// feed). Renders from the visitor's current settings (lib/goodposts.js,
// localStorage-backed) and reports how many posts this particular fetch
// hid. `reload` is called after a toggle/threshold change to re-fetch and
// re-filter — filtering happens client-side on the already-fetched page, so
...
function thresholdKeyOf(n, THRESHOLDS) {
  return Object.keys(THRESHOLDS).find((k) => THRESHOLDS[k] === n) || "balanced";
}
...
async function mountFilterBar(bar, hiddenCount) {
  ...
  bar.querySelector("#goodbar-threshold").addEventListener("change", (e) => {
...
function shellHtml(activePath) {
  ...
  return `
  <div class="mobile-topbar"><span>✅ goodsky</span>${mobileAuthHtml()}</div>
  <div class="shell">
    <nav class="nav">
...
function asideHtml() {
  ...
  <div class="aside-search">
    <span>🔎</span>
    <input id="aside-q" placeholder="Search goodsky" autocomplete="off">
  </div>
...
// their actual follows, not the public Discover feed. `filterState`/`bar` are
// optional — passed by views that show the goodposts toggle bar above the
// feed, so a running hidden-count can be kept across "show more" pages.
async function loadTimeline(box, cursor, filterState, bar) {
  if (!session) {
  ...
  try {
    ...
    if (data.cursor && data.feed.length) {
      ...
      box.querySelector(".load-more").onclick = (e) => {
        ...
        loadTimeline(box, data.cursor, filterState, bar);
      };
    }
  } catch (e) {
...
async function loadFeed(feedUri, box, cursor, filterState, bar) {
  try {
    ...
    if (data.cursor && data.feed.length) {
      ...
      box.querySelector(".load-more").onclick = (e) => {
        e.target.textContent = "Loading…";
        loadFeed(feedUri, box, data.cursor, filterState, bar);
      };
    }
  } catch (e) {
...
// in the background; when it climbs, a toast announces the new ones.
  ...
  try {
    ...
    box.insertAdjacentHTML(
      ...
      data.notifications.map(notificationRow).join("") ||
        centerMsg("Nothing yet", "When someone likes, reposts, follows, or replies, it'll show up here.")
    );
...
// null until the first successful poll, so we never toast about notifications
// that were already sitting there before this tab opened.
...
async function pollNotifications() {
  ...
  try {
    ...
    if (newOnes > 0) showToast(newOnes > 1 ? `${newOnes} new notifications` : "New notification", "ok");
    unreadCount = count;
...
async function CustomFeedView(main, params, args) {
  ...
  try {
    ...
    main.innerHTML =
      ...
      <div class="goodbar" id="goodbar"></div>
...
async function ProfileView(main, params, args) {
  ...
  const shareText = `${profile.displayName || profile.handle} (@${profile.handle}) on goodsky`;
...
async function loadAuthorFeed(actor, filter, box, cursor, filterState, bar) {
  try {
    ...
    if (data.cursor && data.feed.length) {
      ...
      box.querySelector(".load-more").onclick = (e) => {
        e.target.textContent = "Loading…";
        loadAuthorFeed(actor, filter, box, data.cursor, filterState, bar);
      };
    }
  } catch (e) {
...
function metaLikeActionHtml(likeUri, likeCid, count) {
  ...
  return `<span class="act like meta${liked ? " liked" : ""}">
    <span class="like-icon" data-action="like" data-uri="${esc(likeUri)}" data-cid="${esc(likeCid || "")}" title="${liked ? "Caught in your web" : "Like the like — it's legal per lexicon"}">${liked ? "❤️" : "🤍"}</span>
...
function likerRow(a, subjectUri) {
  return `<div class="card-row actor" data-href="${profileUrl(a.handle)}">
    ...
    <a class="meta-like-btn" data-link href="${MOUNT}/meta-likes?liker=${encodeURIComponent(a.did)}&subject=${encodeURIComponent(subjectUri)}" title="Like their like — meta-likes are legal per lexicon">🤍</a>
  </div>`;
}
...
async function ThreadView(main, params, args) {
  ...
  try {
    ...
    const shareText = `${focus.author.displayName || focus.author.handle} (@${focus.author.handle}) on goodsky: "${(focus.record.text || "").slice(0, 100)}"`;
...
function NotFoundView(main) {
  main.innerHTML = headerHtml("Not found") + centerMsg("Nothing here", "That page doesn't exist in goodsky.");
}
...
document.addEventListener("click", (e) => {
  ...
  const hideIcon = e.target.closest("[data-action='hide']");
  if (hideIcon) {
    e.stopPropagation();
    hidePost(hideIcon);
    return;

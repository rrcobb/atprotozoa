// goodsky Worker — goodsky.bisks.net
//
...
//    Bluesky's own link-unfurl cache would show one generic "goodsky" card
...
const SITE_ORIGIN = "https://goodsky.bisks.net";
...
async function renderPost(env: Env, request: Request, handle: string, rkey: string): Promise<Response> {
  ...
  const name = profile.displayName ? `${profile.displayName} (@${profile.handle})` : `@${profile.handle}`;
  const text = post?.record?.text ? truncate(post.record.text, 220) : `A post by ${name} on goodsky.`;
  const image =
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Static assets (JS, CSS, images, the shell itself).

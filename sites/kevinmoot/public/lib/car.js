...
// type(s) the caller wants, instead of paginating a listRecords/getFollows
// cursor walk one page at a time. One request gets every app.bsky.graph.follow
// record an account has ever written, in one shot — no per-page cap, so
// kevinmoot's moot check no longer silently truncates a heavily-followed
// account's follow list (see notes/40-new-site-playbook.md's cee.wtf thread,
// 2026-08-25: "stop using paginated listrecord calls... stop being afraid of
// just loading a ton of data"). Copied from sites/backscroll's lib/car.js
// (itself copied via sites/activitygrid/ngmi/beefcheck) — bfs.js only needs
// each follow record's `subject` DID, not the rkey, but fetchRepoRecordsWithKeys
// returns both so the same file works unmodified.
//
...
// imported across sites), trimmed to just fetchRepoRecordsWithKeys — a
// record's key ("collection/rkey") only exists as an edge label in the
// repo's MST (Merkle Search Tree), not in the record block itself, so a flat
// block scan can't recover it; this walks the MST from the commit's root to

// global-index.js — network-wide view over the hashteams roster collection:
...
// Jetstream subscription for anything written after the page loads. This is
// the entirety of hashteams' "backend" — there is no server-side index at
// all, and no account appears in it unless it wrote the record itself (see
// wrangler.toml for why: the alternative, crawling every DID on the network,
// would seed real accounts into a public roster they never agreed to join).
// Copied from sites/mutualpulse/public/lib/global-index.js (itself from
// sites/witness / sites/shelfguessr / sites/skymash / sites/rateyourbuild /
// sites/steamtags — see notes/ideas/pds-and-lexicons.md, "Tier 3: use
// listReposByCollection"), which already takes the collection name and a
// per-record normalizer, so this one instance indexes
// net.bisks.hashteams.member. normalize() ignores the record's own fields
// entirely and recomputes the team from the entry's own DID, so the roster
...
export class GlobalIndex {
  // collection: full NSID string, e.g. "net.bisks.hashteams.member"
  ...
  constructor(collection, { normalize, onUpdate } = {}) {
    ...
    this.cacheKey = `hashteams:global-index:${collection}:v1`;
    this.entries = new Map();
    // The raw atproto record for each entry, keyed the same as `entries` —
    // persisted to the cache instead of the normalized entry, so a
    // normalize() that isn't idempotent doesn't get silently double-applied
    // across a page reload and corrupt itself.
  ...
  // has necessarily echoed it back), so a writer sees their own act land
  ...
  restoreCache() {
    try {
      ...
      for (const item of parsed.entries.slice(0, MAX_ENTRIES)) {
        ...
        // item.record is the ORIGINAL atproto record — normalize() must only
        // ever see that shape, never its own output, or a non-idempotent
        // normalize corrupts itself on every reload.
  ...
  emit() {
    try {
      ...
      console.error("hashteams global-index render failed", err);
    }
  }
  ...
  async runBackfill() {
    ...
    try {
      ...
      retryMs = 5000;
      console.warn("hashteams global-index backfill failed", err);
    } finally {
      this.backfillRunning = false;
      this.scheduleEmit();

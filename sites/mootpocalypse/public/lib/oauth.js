// oauth.js — browser-side atproto OAuth (public client), for mootpocalypse.
//
// Copied and trimmed from sites/skyclone/public/lib/oauth.js (same repo),
// itself descended from mino.mobi's airchat/oauth/flow.js. Public client:
...
//
// Scope is narrowed to exactly what this game touches: create-only on
// app.bsky.feed.like (biting back — killing a zombie is a real like, no
// unlike/undo mechanic so no delete grant), plus one rpc grant to read each
// moot's author feed *authenticated*, proxied through the player's own PDS to
// the AppView — same rpc:...?aud=did:web:api.bsky.app%23bsky_appview shape as
// sites/verdict's getTimeline-only scope. Authenticated reads are the whole
// point here: only a request made as the player gets real viewer.like state
// back on each post, which is how we know which ones are actually unliked
// zombies instead of guessing. See notes/50-oauth-scopes.md.
...
const ORIGIN = location.origin; // https://mootpocalypse.bisks.net (or localhost in dev)
...
const SCOPE =
  "atproto repo:app.bsky.feed.like?action=create rpc:app.bsky.feed.getAuthorFeed?aud=did:web:api.bsky.app%23bsky_appview";
...
const DB_NAME = "mootpocalypse-oauth";
const STORE = "session";
...
// Make a DPoP-authenticated request to the player's PDS. Refreshes the access
// token when it's expired (or when the server says invalid_token), handles the

// oauth.js — browser-side atproto OAuth (public client), for homemixer.bisks.net.
//
// Copied and trimmed from sites/drivethru/public/lib/oauth.js (same repo),
// itself descended from trigrams/public/lib/oauth.js / mino.mobi's
...
//
// Brand-new subdomain-only site, so no mount-prefix handling like drivethru's.
//
// Scope is narrowed to create-only on app.bsky.feed.generator — this site's
// only write is publishing the feed's own declaration record into the
// signed-in visitor's repo (see publish.js). No account-wide access. Must
// stay in lockstep with client-metadata.json (notes/50-oauth-scopes.md).
...
const ORIGIN = location.origin; // https://homemixer.bisks.net (or localhost in dev)
...
const SCOPE = "atproto repo:app.bsky.feed.generator?action=create";

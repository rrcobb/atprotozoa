// oauth.js — browser-side atproto OAuth (public client), for grudgebook.bisks.net.
//
// Forked from sites/blocknotes/public/lib/oauth.js (same repo), itself
// descended from sites/mutualsky / skyclone / drivethru / mino.mobi's airchat
// oauth/flow.js. Public client:
...
//
// Scope is narrowed to exactly what grudgebook touches: create+delete on this
// site's own net.bisks.grudgebook.entry collection. No block/mute writes, no
// rpc grants — a grudge's "subject" is free text (not a resolved atproto
// actor), so there's nothing else to read or write.
//
// If a narrow repo: scope ever gets rejected by a user's PDS as
...
const ORIGIN = location.origin; // https://grudgebook.bisks.net (or localhost in dev)
const MOUNT = ""; // canonical host is grudgebook.bisks.net, served at the root
...
const SCOPE = "atproto repo:net.bisks.grudgebook.entry?action=create&action=delete";
...
const DB_NAME = "grudgebook-oauth";
const STORE = "session";

...
// cursor walk one page at a time. One request gets typecheck the account's
// *entire* post history in one shot — this is the bulk-read-over-pagination
// standing order (notes/40-new-site-playbook.md's cee.wtf thread,
...
// (itself from sites/activitygrid/sites/beefcheck — copy, don't abstract),
// unchanged: typecheck also wants each post's rkey (for permalinking a
// "swayed by" quote back to the exact post), which a flat block scan can't
// recover — a record's key only exists as an edge label in the repo's MST,
// not in the record block itself. This walks the MST from the commit's root
// to reconstruct every key.

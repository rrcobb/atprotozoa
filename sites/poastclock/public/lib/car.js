...
// sites/padmoot's lib/car.js (copied verbatim, not imported across sites).
//
// fetchRepoRecordsWithKeys below is a second entry point, for callers (like
// this site, which needs each reply's rkey to link back to the post) that
// need a record's rkey/URI, not just its body — a flat block scan can't
// recover that, since a record's key ("collection/rkey") only exists as an
// edge label in the repo's MST (Merkle Search Tree), not in the record block
// itself. It walks the MST from the commit's root to reconstruct every key,
// same one-request trade as fetchRepoRecords.
...
// dag-cbor block — no rkey, no CID, just the record body. Throws on
// network/oversize failure; caller should catch and fall back to a
// listRecords walk.
...
// bare body — for callers that need to address a specific record (e.g. link
// back to the exact post), not just aggregate over its contents. Throws on
// network/oversize/malformed-CAR failure; caller should catch and fall back
// to a listRecords walk.

// oauth.js — browser-side atproto OAuth (public client), for shelfguessr.bisks.net.
//
// Copied and adapted from sites/catspace/public/lib/oauth.js (same repo),
...
//
// Scope: narrowed per notes/50-oauth-scopes.md — shelfguessr only ever
// creates/updates its own net.bisks.shelfguessr.shelf "self" record, creates
// net.bisks.shelfguessr.guess records, and uploads a bookshelf photo blob.
// Must match public/client-metadata.json's `scope` field exactly.
...
const ORIGIN = location.origin; // https://shelfguessr.bisks.net (or localhost in dev)
const MOUNT = "";
...
const SCOPE =
  "atproto repo:net.bisks.shelfguessr.shelf?action=create&action=update repo:net.bisks.shelfguessr.guess?action=create blob:image/*";
...
const DB_NAME = "shelfguessr-oauth";
const STORE = "session";
...
export async function completeLoginIfCallback() {
  ...
  // user started the flow from a non-root page (/upload, /leaderboard) a
  // plain history.replaceState would leave the already-loaded root
  // document's DOM sitting under the wrong URL — a real navigation is
  // needed so the browser actually fetches that page's HTML.

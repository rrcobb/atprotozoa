// oauth.js — browser-side atproto OAuth (public client), for homoskeeter.bisks.net.
//
...
//
// Dedicated subdomain, so no mount prefix — canonical host is
// homoskeeter.bisks.net.
//
// Scope is narrowed to create-only on app.bsky.feed.post (a normal post, from
// the user's own account — the "telepathic transmission" is just a post, same
// as tapping "post" in the Bluesky app) — no account-wide access. Must stay in
// lockstep with client-metadata.json.
...
const DB_NAME = "homoskeeter-oauth";
const STORE = "session";

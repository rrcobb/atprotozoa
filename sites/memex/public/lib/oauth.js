// oauth.js — browser-side atproto OAuth (public client), for memex.bisks.net.
//
// Copied and trimmed from sites/steamtags/public/lib/oauth.js (same repo).
// Public client:
...
// narrowed to create+delete on our own net.bisks.memex.phrase collection
// only — no account-wide access. Must stay in lockstep with
// client-metadata.json. Browsing and
// copying phrases needs no login; signing in just moves your phrasebook off
// this one browser and into your own PDS.
...
const ORIGIN = location.origin; // https://memex.bisks.net (or localhost in dev)
...
const SCOPE = "atproto repo:net.bisks.memex.phrase?action=create&action=delete";
...
export async function login(handleOrDid) {
  ...
  // the auth server's callback can't carry along whatever ?g=<appid> the user
  ...
  // session is established, so signing in from a game page takes you back to
  // that same game instead of dumping you on the search screen.
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page (a game, or the library) the user started the

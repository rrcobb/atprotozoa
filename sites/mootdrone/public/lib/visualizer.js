// visualizer.js — draws the loaded network as a small constellation: one
// glowing node per track, arranged on a circle, pulsing with that track's own
// live level (read off the per-voice analyser synth.js taps after its mute
// gain, so muted/soloed-out tracks naturally go dark). Lines between nodes
// brighten when both ends are audible — the "network" in mootdrone, made visible.
//
// The same canvas doubles as the video source when a clip gets recorded, so
// whatever's drawn here is exactly what ends up in the posted video.
...
export function createVisualizer(canvas) {
  ...
  function draw() {
    ...
    if (!n) {
      ...
      g.textAlign = "center";
      g.fillText("load a network to see it breathe", w / 2, h / 2);

// oauth.js — browser-side atproto OAuth (public client), for mootdrone.bisks.net.
// Copied from trigrams/public/lib/oauth.js (copy, don't abstract) and trimmed to
// mootdrone's needs: it's a single-page site, so the redirect URI is just the
// site root instead of a dedicated /launcher/ path.
//
// PKCE + DPoP only, no client_assertion (public client). Transient auth state →
// sessionStorage; the logged-in session → IndexedDB.
...
// Call on page load. Returns the session if this was an OAuth callback, or
// null if there's no ?code in the URL.
...
// Refresh an expired access token using the stored refresh token. Mutates and

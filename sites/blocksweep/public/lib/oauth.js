// oauth.js — browser-side atproto OAuth (public client), for blocksweep.bisks.net.
//
...
//
// Scope is narrowed to exactly what blocksweep touches: create-only on
// app.bsky.graph.block. That's the whole write surface — membership reads
// (list metadata, list members, profiles) are all public and unauthenticated,
// and this site never deletes/unblocks, so no `&action=delete` is requested.
//
...
const ORIGIN = location.origin; // https://blocksweep.bisks.net (or localhost in dev)
const MOUNT = ""; // canonical host is blocksweep.bisks.net, served at the root
...
const DB_NAME = "blocksweep-oauth";
const STORE = "session";

// oauth.js — browser-side atproto OAuth (public client), for catspace.bisks.net.
//
// Copied and adapted from sites/goodsky/public/lib/oauth.js (same repo),
// descended from mino.mobi's airchat/oauth/flow.js. Public client:
...
//
// Scope: narrowed per notes/50-oauth-scopes.md — catspace only ever creates
// or updates its own net.bisks.catspace.profile "self" record, creates
// net.bisks.catspace.comment records (guestbook), and uploads a cat photo
// blob. Must match public/client-metadata.json's `scope` field exactly.
...
const ORIGIN = location.origin; // https://catspace.bisks.net (or localhost in dev)
const MOUNT = "";
...
const SCOPE =
  "atproto repo:net.bisks.catspace.profile?action=create&action=update repo:net.bisks.catspace.comment?action=create blob:image/*";
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page the user started the login from. The OAuth
  // redirect always comes back to REDIRECT_URI (this site's root), so if the
  // user started the flow from a non-root page (a cat's profile, mid
  // guestbook signature) a plain history.replaceState would leave the
  // already-loaded root document's DOM sitting under the wrong URL — a real
  // navigation is needed so the browser actually fetches that page's HTML.
  if (s.returnTo && s.returnTo !== location.pathname + location.search) {
    location.href = s.returnTo;
    return session;
  }

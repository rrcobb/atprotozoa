// oauth.js — browser-side atproto OAuth (public client), for skullcouncil.bisks.net.
//
// Copied and trimmed from sites/bsky95/public/lib/oauth.js (same repo), itself
// descended from skyclone/mino.mobi. Public client:
...
//
// skullcouncil only ever does one write: create a single app.bsky.feed.post
// reply record on the signed-in visitor's own PDS, after a persona and a dice
// roll have picked its text. Everything it reads (the public feed, a post's
// thread root) goes through the unauthenticated public AppView, so the scope
// below is exactly one narrow create-only grant — no transition:generic, no
// like/repost/timeline rpc grants this site never uses. See
...
const ORIGIN = location.origin; // https://skullcouncil.bisks.net (or localhost in dev)
...
const DB_NAME = "skullcouncil-oauth";
const STORE = "session";

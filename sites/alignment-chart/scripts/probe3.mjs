...
for (const p of people) {
  try {
    ...
    rows.push({ handle: p.handle, posts: parts.length, words, hHits, cHits, mHits, gHits });
    console.log("done", p.handle);
  } catch (e) {
    console.log(p.handle, "ERR", e.message);
  }
}
writeFileSync("/tmp/pack-raw.json", JSON.stringify(rows, null, 2));
console.log("wrote /tmp/pack-raw.json", rows.length, "rows");

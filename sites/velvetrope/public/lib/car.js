...
// cursor walk one page at a time. One request gets a list owner's *entire*
// listitem history in one shot (no per-page cap) — this is the trick
// sites/blocksweep and sites/rollcall use for "give me this list's whole
// membership," applied here to velvetrope's own list-membership reads. See
// notes/40-new-site-playbook.md's cee.wtf thread (2026-08-25) for why this
// beats listRecords/getAuthorFeed/getList for "give me everything."
//
// Copy, don't abstract: this is the keyed variant of the CAR/DAG-CBOR parser
// from sites/beefcheck's lib/car.js, by way of sites/backscroll. One
// deviation from the canonical copy: `onProgress` here is called with a
// second numeric-count argument (matched-records-so-far) alongside the
// message string, so a caller can render a real "N of M" counter instead of
// just echoing the status text — added for velvetrope 2026-09-13
// (@heika.dog asked for a progress counter while scanning a large list).
// The 200-per-tick throttle also drops to 50, since velvetrope's caller
// wants a counter that visibly moves, not just a "still working" ping.

const CAR_MAX_BYTES = 200 * 1024 * 1024; // bail (caller should fall back to the paginated list walk) rather than parse a repo this big in-tab
...
// raw CID bytes, which is what an image-thumbnail path would expect).
...
// back in ascending rkey order (the MST's in-order walk).
...
export async function fetchRepoRecordsWithKeys(pds, did, types, onProgress) {
  ...
  if (onProgress) onProgress(`downloading repo CAR from ${pds} ...`, 0);
  ...
  if (onProgress) onProgress(`parsing ${(buf.byteLength / 1048576).toFixed(1)} MB repo CAR ...`, 0);
    ...
    for (const entry of node.e || []) {
      ...
      if (recBytes) {
        ...
        if (rec && wanted.has(rec.$type)) {
          ...
          if (onProgress && out.length % 50 === 0) onProgress(`walking repo CAR... ${out.length} matching records so far`, out.length);
        }
      }
  ...
  walk(rootMstKey);
  if (onProgress) onProgress(`walked repo CAR — ${out.length} matching record${out.length === 1 ? "" : "s"} found`, out.length);

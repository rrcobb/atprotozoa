// oauth.js — browser-side atproto OAuth (public client), for skeetin.bisks.net.
//
// Copied and adapted from sites/skyclone/public/lib/oauth.js (same repo),
// itself descended from drivethru's / slop-shop's / mino.mobi's
// airchat/oauth/flow.js. Public client:
...
//
// skeetin is a dedicated subdomain (skeetin.bisks.net), not path-mounted, so
// there's no MOUNT prefix to bake into client_id/redirect_uri.
//
// Scope is narrowed to exactly what skeetin's login unlocks (@antiali.as's
// "full atproto login and feed browsing, posting, liking" ask): create-only
// on app.bsky.feed.post (Start a post), create+delete on app.bsky.feed.like
// (Endorse / un-Endorse), plus one rpc grant for the real home timeline
// (My Feed), proxied through the user's own PDS to the AppView. Everything
// else skeetin reads (profiles, other people's feeds, What's Hot) goes
// through the public unauthenticated AppView and needs no scope at all —
// see notes/50-oauth-scopes.md.
//
// If a narrow rpc:/repo: scope ever gets rejected by a user's PDS as
...
const ORIGIN = location.origin; // https://skeetin.bisks.net (or localhost in dev)
...
const SCOPE =
  "atproto repo:app.bsky.feed.post?action=create repo:app.bsky.feed.like?action=create&action=delete rpc:app.bsky.feed.getTimeline?aud=did:web:api.bsky.app%23bsky_appview";
...
// (getTimeline's limit/cursor) straight through without hand-rolling two URLs.

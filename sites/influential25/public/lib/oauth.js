// oauth.js — browser-side atproto OAuth (public client), for influential25.bisks.net.
//
// Copied and trimmed from sites/socialcredit/public/lib/oauth.js (same repo,
// same pattern, itself from sites/quadrants via padmoot). Public client:
...
// narrowed to our own net.bisks.influential25.vote collection, create-only —
// no account-wide access, no update/delete (a nomination is permanent once cast).
...
const ORIGIN = location.origin; // https://influential25.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is influential25.bisks.net — brand new site, no legacy path route
...
export const SCOPE = "atproto repo:net.bisks.influential25.vote?action=create";
...
const DB_NAME = "influential25-oauth";
const STORE = "session";
...
export async function login(handleOrDid) {
  ...
  //
  // redirect_uri is a fixed "/" (must match client-metadata.json exactly), so
  // the auth server's callback can't carry along whatever page the user was
  // on when they clicked "sign in". We stash that here as `returnTo` and
  // completeLoginIfCallback() navigates back to it once the session is
  // established, so signing in from a profile page takes you back to that
  // same profile instead of dumping you on the home page.
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page the user started the login from, not the
  // bare redirect_uri MOUNT + "/" the auth server sent us to. This site has
  // three real documents (/, /leaderboard/, /p/<handle>), not one SPA shell,
  // so a returnTo that isn't the page we're already on needs a real
  // navigation (location.replace) — a plain history.replaceState would only
  // rewrite the URL bar while leaving this document's (wrong) HTML on
  // screen. The session is already in IndexedDB, so the fresh load picks it
  // straight up via getSession(), no re-auth.
  const target = s.returnTo || `${MOUNT}/`;

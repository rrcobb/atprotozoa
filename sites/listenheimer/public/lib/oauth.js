// oauth.js — browser-side atproto OAuth (public client), for listenheimer.bisks.net.
//
// Copied and trimmed from sites/mootfluence/public/lib/oauth.js (same repo,
...
// only needed for the "build the moderation list" step — reading a post's
// likers is anonymous, public-AppView reads. Scope is narrowed to exactly
// the two collections a modlist write touches (the list itself + one
// listitem per liker), create-only — no account-wide access, no
// update/delete. Must stay in lockstep with client-metadata.json.
...
const ORIGIN = location.origin; // https://listenheimer.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is listenheimer.bisks.net — brand new site, no legacy path route
...
export const SCOPE =
  "atproto repo:app.bsky.graph.list?action=create repo:app.bsky.graph.listitem?action=create";
...
const DB_NAME = "listenheimer-oauth";
const STORE = "session";

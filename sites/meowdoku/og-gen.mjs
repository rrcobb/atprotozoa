// Generates public/og.png — the Open Graph preview card for meowdoku, so a
// shared link auto-renders a picture of the game in Bluesky / other unfurlers.
//
// Hand-drawn SVG: the smug tabby mascot (same shapes as the in-page
// #mascot-yay symbol), title/tagline, and a real generated killer-sudoku
// board — actual cages and sums from engine.js, not a mockup — with the
// classic dashed-cage look. Rasterised with @resvg/resvg-js (pure native
...
// sites/claudoku/og-gen.mjs). Re-run by hand if you change the artwork.

import { Resvg } from "@resvg/resvg-js";
import { writeFileSync } from "node:fs";
import { fileURLToPath } from "node:url";
import Meowdoku from "./public/engine.js";
...
const puzzle = Meowdoku.generatePuzzle("og-card-v1");
...
const mascot = `
  <g transform="translate(${mx - 50*ms},${my - 50*ms}) scale(${ms})">
    <path d="M82 78 Q100 68 92 46" stroke="url(#mgrad)" stroke-width="9" fill="none" stroke-linecap="round"/>
    <ellipse cx="10" cy="50" rx="6" ry="10" fill="url(#mgrad)" transform="rotate(-30 10 50)"/>
    <ellipse cx="90" cy="50" rx="6" ry="10" fill="url(#mgrad)" transform="rotate(30 90 50)"/>
    <path d="M22 34 L14 10 L38 26 Z" fill="url(#mgrad)"/>
    <path d="M78 34 L86 10 L62 26 Z" fill="url(#mgrad)"/>
    <path d="M24 30 L19 15 L34 25 Z" fill="#ffcfae"/>
    <path d="M76 30 L81 15 L66 25 Z" fill="#ffcfae"/>
    <rect x="18" y="30" width="64" height="56" rx="28" fill="url(#mgrad)"/>
    <path d="M28 34 L24 24" stroke="#a5502f" stroke-width="3" stroke-linecap="round" opacity=".6"/>
    <path d="M72 34 L76 24" stroke="#a5502f" stroke-width="3" stroke-linecap="round" opacity=".6"/>
    ...
    <path d="M50 60 L46 64 L54 64 Z" fill="#e08fa0"/>
    <path d="M38 68 Q50 82 62 68" stroke="#2a160f" stroke-width="3.6" fill="none" stroke-linecap="round"/>
    <path d="M14 58 L30 60 M13 66 L30 65 M86 58 L70 60 M87 66 L70 65" stroke="#ffdca8" stroke-width="1.6" fill="none" stroke-linecap="round" opacity=".85"/>
  </g>`;
...
const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  ...
  <text x="74" y="470" font-family="JetBrains Mono" font-weight="700"
    font-size="66" letter-spacing="1" fill="${ORANGE}">meowdoku</text>
  <text x="74" y="512" font-family="JetBrains Mono" font-size="21"
    fill="${DIM}">killer sudoku with a smug tabby sidekick</text>
  ...
  <text x="${W - 74}" y="588" text-anchor="end" font-family="JetBrains Mono"
    font-size="16" fill="${TEAL}">meowdoku.bisks.net</text>
</svg>`;

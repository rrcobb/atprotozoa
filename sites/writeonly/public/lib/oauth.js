// oauth.js — browser-side atproto OAuth (public client), for writeonly.bisks.net.
//
// Copied and trimmed from drivethru/public/lib/oauth.js (same repo), itself
// descended from trigrams/public/lib/oauth.js / mino.mobi's airchat/oauth/flow.js.
// Public client:
...
//
// Scope is narrowed to create-only on app.bsky.feed.post plus image blob
// uploads (a normal post with an optional photo, same as tapping "post" in
// the Bluesky app) — no account-wide access, and critically, no read scope
// at all. That last part is the entire point of this site: the OAuth token
// this page ever holds is structurally incapable of reading your timeline,
// notifications, or even the post it just sent — not a UI choice hiding
// those views, an absent grant. Must stay in lockstep with
...
const ORIGIN = location.origin; // https://writeonly.bisks.net (or localhost in dev)

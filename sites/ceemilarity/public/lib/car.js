...
// the caller wants, instead of paginating app.bsky.feed.getAuthorFeed one page
// at a time. One request gets the whole history in the same download (no
// cursor walk, no page cap) — see sites/activitygrid, where this trick first
// shipped, for the full writeup of why it beats a paginated feed walk for
// "give me this person's whole history."
//
// @bisks.net asked ceemilarity (née styletwin) to "use cars instead of
// looping on the read endpoint" — this file is that swap. public/index.html
// and src/index.ts both try this first and only fall back to the old
// getAuthorFeed loop if the CAR fetch/parse fails (private PDS with no CORS,
// oversized repo, etc).
//
// Copy, don't abstract: this is the same CAR/DAG-CBOR parser as
// sites/trigrams's lib/car.js (copied verbatim, not imported across sites).

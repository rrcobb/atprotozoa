// oauth.js — browser-side atproto OAuth (public client), for vouch.bisks.net.
//
// Copied and trimmed from sites/hyperobject/public/lib/oauth.js (same repo),
// itself descended from clusterpedia/padmoot/trigrams/mino.mobi's
...
// narrow grant — create+update+delete on net.bisks.vouch.vouch, the only
// collection this site ever writes (a vouch record is create-or-update via
// putRecord since rkey = subject DID makes re-vouching idempotent; un-vouching
// is a delete) — not the old `transition:generic` app-password-equivalent
...
const ORIGIN = location.origin; // https://vouch.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is vouch.bisks.net — no path mount
...
const SCOPE = "atproto repo:net.bisks.vouch.vouch?action=create&action=update&action=delete";

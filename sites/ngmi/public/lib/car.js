...
// type(s) the caller wants, instead of paginating com.atproto.repo.listRecords
// one page at a time. One request gets every collection in the same download
...
// from sites/beefcheck's lib/car.js (copied verbatim, not imported across
// sites), trimmed to just fetchRepoRecordsWithKeys — ngmi needs every post's
// rkey to link a citation back to the exact post, which a flat block scan
...
// and keeps the cheaper "unwrap and ignore" behavior.
...
// so a caller can address or link back to the exact record. Throws on

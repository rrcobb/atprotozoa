// oauth.js — browser-side atproto OAuth (public client), for bisks.net/vibe-store.
//
...
const DB_NAME = "vibe-store-oauth";
const STORE = "session";
...
export async function login(handleOrDid) {
  ...
  // we stash the page's actual path+query as `returnTo` and
  // completeLoginIfCallback() navigates back to it once the session is
  // established, rather than always landing on the bare mount root.

...
const switches = ["countability", "number", "definiteness", "case", "animacy", "properness"];

const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}">
  <rect width="${W}" height="${H}" fill="${BG}"/>
  <rect x="0" y="0" width="10" height="${H}" fill="${ACCENT}"/>

  <text x="70" y="110" font-family="JetBrains Mono" font-weight="700" font-size="16" letter-spacing="3" fill="${ACCENT}">A LITTLE LINGUISTICS PAGE</text>
  <text x="70" y="195" font-family="JetBrains Mono" font-weight="800" font-size="84" fill="${INK}">nounlab</text>

  <text x="70" y="250" font-family="JetBrains Mono" font-size="20" fill="${MUTED}">pick a noun, flip a switch, watch the sentence change</text>

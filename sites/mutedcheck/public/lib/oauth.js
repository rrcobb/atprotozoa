// oauth.js — browser-side atproto OAuth (public client), for mutedcheck.bisks.net.
//
// Forked from sites/mutechella/public/lib/oauth.js (same repo), trimmed to a
// single read-only rpc grant. Public client:
...
//
// mutedcheck never writes anything — it signs you in only to read your own
// muted-words list via app.bsky.actor.getPreferences. Unlike getMutes (which
// is AppView state proxied through the PDS, hence sibling sites' `?aud=...`),
// preferences are explicitly PDS-private and never synced to the AppView
// (see the lexicon doc), so this call goes straight to the user's own PDS
// with no atproto-proxy header — no `aud` on the scope. No repo: grants, no
// blob: grants.
//
// If narrow rpc: scope ever gets rejected by a user's PDS as invalid_scope,
// the safe fallback is reverting this SCOPE and client-metadata.json's scope
// back to `atproto transition:generic` for this one site — see
...
const ORIGIN = location.origin; // https://mutedcheck.bisks.net (or localhost in dev)
const MOUNT = ""; // canonical host is mutedcheck.bisks.net, served at the root
...
const SCOPE = "atproto rpc:app.bsky.actor.getPreferences";
...
const DB_NAME = "mutechella-oauth";
const STORE = "session";

// global-index.js — network-wide view over a skymash collection:
...
// Copied and adapted from sites/rateyourbuild/public/lib/global-index.js
// (itself from sites/steamtags — see notes/ideas/pds-and-lexicons.md,
// "Tier 3: use listReposByCollection"), generalized here to take the
// collection name and a per-record normalizer so the same class can index
// both net.bisks.skymash.nomination and net.bisks.skymash.vote — this is
...
export class GlobalIndex {
  ...
  emit() {
    try {
      ...
      console.error("skymash global-index render failed", err);
    }
  }
  ...
  async runBackfill() {
    ...
    try {
      ...
      retryMs = 5000;
      console.warn("skymash global-index backfill failed", err);
    } finally {
      this.backfillRunning = false;
      this.scheduleEmit();

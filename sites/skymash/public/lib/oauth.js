// oauth.js — browser-side atproto OAuth (public client), for skymash.bisks.net.
//
// Copied and trimmed from quadrants/public/lib/oauth.js (same repo). Public client:
...
const ORIGIN = location.origin; // https://skymash.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is skymash.bisks.net — brand-new site, no path mount
...
export async function login(handleOrDid) {
  ...
  // the auth server's callback can't carry along whatever chart the user was
  // looking at when they clicked "sign in". We stash that here as `returnTo`
  // and completeLoginIfCallback() navigates back to it once the session is
  // established, so signing in from a chart page takes you back to that same
  // chart instead of dumping you on the home page.
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever page (chart or home) the user started the login

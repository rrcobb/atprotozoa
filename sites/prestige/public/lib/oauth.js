// oauth.js — browser-side atproto OAuth (public client), for prestige.bisks.net.
//
// Copied and trimmed from sites/hyperobject/public/lib/oauth.js (same repo),
// itself descended from padmoot/velvetrope/mino.mobi's flow.js. Public client:
...
//
// The client_id is the URL of our static client-metadata.json. Scope is a
// single narrow create-only `repo:` grant on the one collection this site
// writes (net.bisks.prestige.link) — not the old `transition:generic`
// app-password-equivalent scope, which would hand us read/write over the
// signer's *entire* repo. `atproto` alone just confirms there's a real
// signed-in account.
...
const ORIGIN = location.origin; // https://prestige.bisks.net (or localhost in dev)
export const MOUNT = ""; // canonical host is prestige.bisks.net — no path mount
...
const SCOPE = "atproto repo:net.bisks.prestige.link?action=create";
...
export async function login(handleOrDid) {
  ...
  // established, so signing in from any view takes you back to it.
...
export async function completeLoginIfCallback() {
  ...
  // Land back on whatever view the user started the login from, not the bare

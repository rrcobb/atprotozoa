// mutualpulse Worker — served at the root of mutualpulse.bisks.net.
// Every act is a plain net.bisks.mutualpulse.act record the browser writes
// to the *poster's own* PDS; the pulse board and feed are computed
// client-side by replaying every repo's acts network-wide
// (public/lib/global-index.js). No KV, no Durable Object, no server-side
// state at all — this Worker is a pure static-asset passthrough, with one
// exception below.
//
// /pulse/<did>/<rkey> — a permalink to one logged act
...
// every act, so a shared pulse would all unfurl as one identical card. This
// route resolves the poster's PDS, fetches the actual record with
// com.atproto.repo.getRecord (a public, unauthenticated read — same call the
// client makes directly for the same permalink, see fetchActDirect in
// public/app.js), and stamps real og:title/description/url onto the static
// shell before serving it. Same resolve-then-stamp pattern as
// sites/witness's /entry/<did>/<rkey>.
export interface Env {
  ASSETS: { fetch: (req: Request) => Promise<Response> };
}

const COLLECTION = "net.bisks.mutualpulse.act";
const CATEGORIES = new Set(["food", "ride", "repair", "care", "skill", "shelter", "other"]);
...
async function renderActPage(env: Env, request: Request, did: string, rkey: string): Promise<Response> {
  ...
  if (!record || !CATEGORIES.has(record.category) || typeof record.text !== "string" || !record.text.trim()) {
    // Unresolvable DID, deleted record, or malformed data — still serve the
    // live shell rather than a dead link; the client's own fetchActDirect
    // surfaces the real "couldn't load that pulse" state.
    return base;
  }

  const handle = await getHandle(did);
  const who = handle ? `@${handle}` : did.slice(0, 16) + "…";
  const title = `mutualpulse: ${record.text.trim()}`;
  const bits = [record.category as string];
  if (record.place) bits.push(String(record.place));
  const desc = `${bits.join(" · ")} — logged by ${who} on mutualpulse.bisks.net, a live pulse-board of mutual aid.`;
  ...
  const stamped = stampMeta(
    ...
    title,
    desc,
    `https://mutualpulse.bisks.net/pulse/${encodeURIComponent(did)}/${encodeURIComponent(rkey)}`,
  );
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    const m = url.pathname.match(/^\/pulse\/([^/]+)\/([^/]+)\/?$/);
    if (m) return renderActPage(env, request, decodeURIComponent(m[1]), decodeURIComponent(m[2]));

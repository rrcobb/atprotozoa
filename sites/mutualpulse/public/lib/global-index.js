// global-index.js — network-wide view over the mutualpulse collection:
...
// the entirety of mutualpulse's "backend" — there is no server-side index at
// all, so the pulse board and feed are only ever a replay of whatever's
// really sitting on everyone's own PDS.
// Copied verbatim from sites/witness/public/lib/global-index.js (itself from
// sites/shelfguessr / sites/skymash / sites/rateyourbuild / sites/steamtags —
// see notes/ideas/pds-and-lexicons.md, "Tier 3: use listReposByCollection"),
// which already takes the collection name and a per-record normalizer, so
// this one instance indexes net.bisks.mutualpulse.act.
//
...
export class GlobalIndex {
  // collection: full NSID string, e.g. "net.bisks.mutualpulse.act"
  ...
  constructor(collection, { normalize, onUpdate } = {}) {
    ...
    this.cacheKey = `mutualpulse:global-index:${collection}:v1`;
  ...
  emit() {
    try {
      ...
      console.error("mutualpulse global-index render failed", err);
    }
  }
  ...
  async runBackfill() {
    ...
    try {
      ...
      retryMs = 5000;
      console.warn("mutualpulse global-index backfill failed", err);
    } finally {
      this.backfillRunning = false;
      this.scheduleEmit();

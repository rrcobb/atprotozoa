// oauth.js — browser-side atproto OAuth (public client), for bisks.net/skyclone.
//
// Copied and adapted from sites/drivethru/public/lib/oauth.js (same repo),
// itself descended from slop-shop/public/lib/oauth.js / mino.mobi's
...
//
// Path-mounted (not a dedicated subdomain), so the client_id/redirect_uri need
// the /skyclone mount prefix baked in — location.origin alone drops it. MOUNT
// is detected from the current path so local `wrangler dev` (served at the
// root, no prefix) still boots, even though OAuth itself only really works
